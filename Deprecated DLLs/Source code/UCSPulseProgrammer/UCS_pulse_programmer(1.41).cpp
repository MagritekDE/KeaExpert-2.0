...
1.41/2 -------------------------------
  ...
  2. Added the rampedrf commands based on the waltzdec commands but which can be used independently.
   ...
   Last modified 14 June 2022 CDE
...
EXPORT void  ListCommands(void)
{
   TextMessage("\n\n   UCS (Spinsolve version) Pulse Programmer DLL module (V%1.2f)\n\n",VERSION);
...
EXPORT bool GetCommandSyntax(char* cmd, char* syntax)
{
 ...
 //  else if(!strcmp(cmd,"execwait"))          strcpy(syntax,"execwait(program,arguments)");
   ...
   else if(!strcmp(cmd,"skiponzero"))           strcpy(syntax,"skiponzero(name, value)");
...
// Open the asm file for append
   ...
   fprintf(fp,"\n        move    a1,x:TXF00");               // the main Ch-1 Tx frequency location
   fprintf(fp,"\n        move    x:FX%s_1,a1",freq.Str()+1);
   fprintf(fp,"\n        move    a1,x:TXF01");
   fprintf(fp,"\n        rep     #27");
   fprintf(fp,"\n        nop");
...
// Get second frequency
 ...
 //  fprintf(fp,"\n        move    a1,x:TXF00");               // the main Tx frequency location for channel 2
...
short RampedRFPulse(DLLParameters* par, char* args)
{
   ...
   else  if (channel == "2") // Channel 2 code
   {
      fprintf(fp, "\n\n;");
      fprintf(fp, "\n;***************************************************************************");
...
short WaltzDecSetUp(DLLParameters* par,char *args)
{
   ...
   if(channel == "1")
   {
      ...
      fprintf(fp,"\n        lsl     #2,a                    ; Divide amplitude by 16 (and left shift x2 for MSB alignment)"); 
      ...
      fprintf(fp,"\n        move    #$000000,a1"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      fprintf(fp,"\n        move    #$002003,a1             ; step rate 32 (128ns), endAddr 36"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
...
short SpoilerXYZ(DLLParameters* par, char* args)
{
   ...
   arg.Format("%s", dSpoil);     MakeADelay(par, arg.Str());
   ...
   arg.Format("%s", dStab);      MakeADelay(par, arg.Str());
...
void WaltzQ(DLLParameters* par, CText &channel,CText &loops,CText &amplitude,CText &duration90,CText &duration180,CText &duration270,CText &duration360,CText &phasePlus,CText &phaseMinus)
{
   ...
   arg = "4.4";                                             MakeADelay(par,arg.Str());
...
// Get mode
   ...
   {
      if((nrArgs = ArgScan(par->itfc,args,3,"mode, nr points, shift factor","eeee","qqqq",&mode,&nrPnts,&shift,&duration)) < 0)
         return(nrArgs);
