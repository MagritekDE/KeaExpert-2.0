...
#define DSP_MEMORY_ADRS 10000 // Memory location 

//#define VERSION 1.22 // 10-November-20
#define VERSION 1.23 // 22-January-21
...
1.19 ------------------------------
	...
	1. Changed the help file location to <documentation>\Macros\Pulse programming.
...
1.21 ------------------------------
	...
	1. Fixed the timing on the 1 channel shaped pulse so it could work with 2us min step time (not 2.1!) both
	   with the current frequency and a defined frequency.

1.22 ------------------------------

	1. Added possibilty to use amplitude tables with the shim16 command.
	2. Corrected the zero crossing offset problem in the shim16 command.

1.23 ------------------------------

	1. Extended pulse and txon/off to work with a 3rd channel (experiments so far)

  // Last modified 10 November 2020 CDE
   Last modified 22 Jan 2021 CDE
...
EXPORT bool GetCommandSyntax(char* cmd, char* syntax)
{
   ...
   else if(!strcmp(cmd,"acquireoff"))        strcpy(syntax,"acquireoff(mode, number points:n)");
 ...
 //  else if(!strcmp(cmd,"execwait"))          strcpy(syntax,"execwait(program,arguments)");
   ...
   else if(!strcmp(cmd,"shapedrfdual"))      strcpy(syntax,"shapedrfdual(atable:t, stable:t, phase1:p, phase2:p, table_size:n, table_step_duration:d)");
...
// Wait for parameters to update
   ...
   fprintf(fp,"\n        and     #$FFBCF5,a               ; Switch off Proton and carbon");
...
//			fprintf(fp,"\n        or      #$000300,a              ; Internal HPA (carbon)");
//
//		fprintf(fp,"\n        move    a1,y:TTL                ; Save new TTL word");
//		fprintf(fp,"\n        move    a1,x:FPGA_TTL           ; Update TTL RF remains the same");
//
...
short ShapedRFSingleChannel(DLLParameters* par, char *args)
{
  ...
  // CArg carg;
   const int pgoOffset =  5;
   //const int firstDelay = 201;
   //const int midDelay =   152;
   //const int endDelay =   98;
   const int firstDelay = 198;
   const int midDelay =   149;
   const int endDelay =   98;
   if((nrArgs = ArgScan(par->itfc,args,6,"mode, atable, stable, phase, table_size, table_step [,frequency]","eeeeeee","qqqqqqq",&channel,&atable,&ptable,&phase,&size,&duration,&freq)) < 0)
      return(nrArgs);
   if(!parList)
   {
      ErrorMessage("Pulse sequence not initialised");
...
/**********************************************************************
     ...
     "1" external TTL (TTL 0x4000) Ch 1 RF (Proton)
     "2" external TTL (TTL 0x0300) Ch 2 RF (Carbon)
     "w1" external TTL (TTL 0x5000) Ch 1 RF (Proton Wobble)
     "w2" external TTL (TTL 0x0700) Ch 2 RF (Carbon Wobble)
...
//      fprintf(fp,"\n        or      #$00300,a              ; Carbon");
//   else if(channel == "w2")
//      fprintf(fp,"\n        or      #$00700,a              ; Carbon + Duplexer relay");
//   else if(channel == "n1" | channel == "n2")
...
//   else
//   {
//      if(channel == "1" || channel == "w1" || channel == "n1")
//      {
//		   fprintf(fp,"\n        move    x:TXF00,a1");
...
//		   fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
//      }
//      else if(channel == "2" || channel == "w2" || channel == "n2")
//      {
//		   fprintf(fp,"\n        move    x:TXF00,a1");
...
//   fprintf(fp,"\n        move    y:TTL,a1                ; Load the current TTL word");
//
//   if(channel == "2" || channel == "w2" || channel == "n2")
...
/**********************************************************************
     ...
     "1" external TTL (TTL 0x4000) Ch 1 RF (Proton)
     "2" external TTL (TTL 0x0300) Ch 2 RF (X1)
     "3" external TTL (TTL 0x2000) Ch 3 RF (X2)
     "w1" external TTL (TTL 0x5000) Ch 1 RF (Proton Wobble)
     "w2" external TTL (TTL 0x0700) Ch 2 RF (X1 Wobble)
     "w3" external TTL (TTL 0x0800) Ch 3 RF (X2 Wobble)
...
short SwitchOnOneTxChannel(DLLParameters* par, char *args, short nrArgs)
{
   ...
   else if(channel == "n1" || channel == "n2" || channel == "n3")
      fprintf(fp,"\n        or      #$00000,a               ; No TTL line");
...
/**********************************************************************
     ...
     "1" external TTL (TTL 0x4000) Ch 1 RF (Proton)
     "2" external TTL (TTL 0x0300) Ch 2 RF (X1)
     "3" external TTL (TTL 0x2000) Ch 2 RF (X2)
     "w1" external TTL (TTL 0x5000) Ch 1 RF (Proton Wobble)
     "w2" external TTL (TTL 0x0700) Ch 2 RF (X1 Wobble)
     "w3" external TTL (TTL 0x0800) Ch 3 RF (X2 Wobble)
...
//      fprintf(fp,"\n;"); 
//      fprintf(fp,"\n        move    y:TTL,a1                ; Load the current TTL state");
//      fprintf(fp,"\n        and     #$FFBCF5,a              ; Switch off all RF TTL lines");
...
// Send to appropriate FPGA channel
   if(channel == "1" || channel == "w1" || channel == "n1")
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
   else if(channel == "2" || channel == "w2" || channel == "n2" || channel == "3" || channel == "w3" || channel == "n3")
      fprintf(fp,"\n        move    a1,x:FPGA_DDS2_Pro0");
   else
   {
...
short SelectFrequency(Interface *itfc, FILE*fp, CText freq, CText channel)
{
   ...
   {
	   if(channel == "1" || channel == "n" || channel == "w1")
	   {
		   fprintf(fp,"\n        move    x:FX%s_0,a1",freq.Str()+1);
		   fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
		   fprintf(fp,"\n        move    x:FX%s_1,a1",freq.Str()+1);
