/* Template for DSP Pulse programs */
...
int main( void )
{
	...
	"); 
	setupDSP();
	*p30 = Read_FPGA(FPGA_HardVer); 
	*p31 = Read_FPGA(FPGA_SoftBuild);
	 
	setPDir(1); /* Turn on GPIO */ 
	/*Write_FPGA(FPGA_JTAG_WR,0); 
  	Write_FPGA(FPGA_JTAG_Con,1);  
  	Write_FPGA(FPGA_JTAG_WR,0x8);
  	Write_FPGA(FPGA_JTAG_WR,0x0);*/
  	setreadByteAddress(0x10000);
  	/* Start XSVF code */
  	*p29 = xsvfgo();
  	*p28 = (unsigned char) getreadByteAddress();
    setPDir(0); /* Turn off GPIO */
	/*	Write_FPGA(FPGA_JTAG_Con,0); /* disable JTAG */ 
