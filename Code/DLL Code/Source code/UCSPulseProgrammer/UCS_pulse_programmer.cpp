...
Spinsolve (UC) spectrometer 

SpinsolveExpert Version
...
#define DSP_MEMORY_ADRS 10000 // Memory location for NMR data
...
1.23 ------------------------------ (note this skips changes in made in Spinsolv SW version)
...
1.27 -------------------------------
   ...
   0. Merges back changes made in Expert version. (1.19-1.22)
   1. Added a dual pulse version of the shaped RF pulse. Requires interleaved amp and phase tables and a
      minimum step size of 4.5us. Number of points is half the total points in the amp or phase tables as
      this corresponds to the number of points per channel

   2. Fixed the timing on the 1 channel shaped pulse so it could work with 2us min step time (not 2.1!) both
      with the current frequency and a defined frequency.
   3. Added possibilty to use amplitude tables with the shim16 command.
   4. Corrected the zero crossing offset problem in the shim16 command. ???
   5. Changed a few function names (MakeRFPulse->RFPulse, EitherChannelRFPulse->SingleChannelRFPulse
      BothChannelsRFPulse->DualChannelRFPulse.

1.28 -------------------------------

   1. Added channel 2 Tx frequency to default parameter list.
   2. Change 1 means modifications to settxfrequency(ies) commands.
   3. Change to inctxfreq command - now has channel number as an option.

1.29 -------------------------------
   1. STD Software version of 1.28

*** Even numbers now for Expert versions odd for Std SW *****

1.30 ------------------------------- 
   1. New command rampedpulse
   2. Removed error message if DSP connection not found

1.32 -------------------------------
   1. Test version with waltzdec and helper functions waltzstart, waltzend, waltzdecpulse waltzdecsetup

1.34 -------------------------------
   1. stoplockandsat command added. This sends a message to the lock board to stop the lock and generate
      a saturation pulse. Then it waits for a T1 signal on the backplane to show this is finished. The
      rest of the sequence then proceeds

1.36 -------------------------------
  1. Addition of the skiponzero endskip commands to skip command

1.38 -------------------------------
  1. Addition of the wetsuppression and spoilerxyz command

1.40 -------------------------------
  1. Addition of the shapedshim command

1.41/2 -------------------------------
  1. Modified acquire (integrate) so that the memory address is saved
     between calls. 
  2. Added the rampedrf commands based on the waltzdec commands but which can be used independently.

1.43 -------------------------------
  1. Added shapedrf2 and dualshapedrf2 commands for compatibility with the FX3 code
  2. Renamed several functions to make them easier to find.
  3. skiponzero wasn't clearing register a before comparison - sometimes this resulted in command being ignored.

 1.44 -------------------------------
  1. Gave skiponzero/endskip the alternative name iftrue/endiftrue

 1.45 --------------------------------
  1. Modified sum acquire mode to include a shift scaling factor. (current a dummy parameter for DSP - used by FX3 code).
  2. Modified sum acquire mode to accept 'stack' as an alternative to 'sum'
  3. The cleardata command now accepts a dummy second parameter sums only used by FX3 code.

  1.46 -------------------------------
  1. Support for user variable in Waitfortrigger (but just for FX3)

  Last modified 18 September 2024 CDE
...
short RFPulseBoost(DLLParameters*,char*);
short RampedRFPulseStart(DLLParameters* par, char* args);
short RampedRFPulse(DLLParameters*, char*);
short RampedRFPulseEnd(DLLParameters* par, char* args);
short RampedPulse(DLLParameters* par, char* args);
...
short SelectPhaseRampedPulse(Interface* itfc, FILE* fp, CText phase, CText channel);
...
short ShimRamp(DLLParameters* par, char *args);
...
short ShapedShim(DLLParameters*, char*);
...
short WetSuppression(DLLParameters*, char*);
short SpoilerXYZ(DLLParameters* par, char* args);
...
EXPORT short  AddCommands(char *command, char *parameters, DLLParameters *dpar)
{
   ...
   else if(!strcmp(command,"rampedpulse"))       r = RampedPulse(dpar, parameters);
   else if(!strcmp(command,"rampedrfpulse"))     r = RampedRFPulse(dpar, parameters);
   else if(!strcmp(command,"rampedrfstart"))     r = RampedRFPulseStart(dpar, parameters);
   else if(!strcmp(command,"rampedrfend"))       r = RampedRFPulseEnd(dpar, parameters);
   else if(!strcmp(command,"rampedrfsetup"))     r = WaltzDecSetUp(dpar, parameters);
   ...
   else if(!strcmp(command,"shimramp16"))        r = ShimRamp(dpar, parameters);
   else if(!strcmp(command,"shapedshim"))        r = ShapedShim(dpar, parameters);
   ...
   else if(!strcmp(command,"spoilerxyz"))        r = SpoilerXYZ(dpar, parameters);
   ...
   else if(!strcmp(command,"wetsuppress"))       r = WetSuppression(dpar, parameters);
...
EXPORT void  ListCommands(void)
{
   TextMessage("\n\n   UCS (Expert version) Pulse Programmer DLL module (V%1.2f)\n\n",VERSION);
 ...
 //  TextMessage("   execwait ........... execute a program and wait for it to exit\n");
   ...
   TextMessage("   rampedrfstart ...... select ramped rf mode\n");
   TextMessage("   rampedrfend ........ deselect ramped rf mode\n");
   TextMessage("   rampedrfsetup ...... setup ramped pulse mode\n");
   TextMessage("   rampedrfpulse ...... generate an RF pulse with ramped ends\n");
   ...
   TextMessage("   shapedshim ......... generate a shaped shim current for up to 3 shims\n");
   ...
   TextMessage("   wetsuppress ........ core commands of the WET suppression sequence\n");
...
EXPORT bool GetCommandSyntax(char* cmd, char* syntax)
{
   ...
   else if(!strcmp(cmd,"dualshapedrf1"))        strcpy(syntax,"dualshapedrf1(ptable:t, phase1 : p, phase2 : p,  table_size : n, table_step_duration : d)");
 ...
 //  else if(!strcmp(cmd,"execwait"))          strcpy(syntax,"execwait(program,arguments)");
   ...
   else if(!strcmp(cmd,"pulse"))                strcpy(syntax,"pulse(channel (1/2), amp:a/t, phase:p/t, duration:d [,freq:f] OR pulse(1 ,a/t1, p/t1, f1, 2 ,a/t2, p/t2, f2, d])");
   else if(!strcmp(cmd,"pulseboost"))           strcpy(syntax,"pulseboost(channel (1/2), boost:n (1/0))");
   else if(!strcmp(cmd,"rampedrfend"))          strcpy(syntax,"rampedrfend(channel:n)");
   else if(!strcmp(cmd,"rampedrfpulse"))        strcpy(syntax,"ramprfpulse(channel (1/2), dummy_amplitude: a, phase:p/t, duration:d");
   else if(!strcmp(cmd,"rampedrfsetup"))        strcpy(syntax,"rampedrfsetup(channel (1/2), amp:a, [frequency:f]");
   else if(!strcmp(cmd,"rampedrfstart"))        strcpy(syntax,"ramprfstart(channel:n, duration270:d)");
   ...
   else if(!strcmp(cmd,"shapedshim"))           strcpy(syntax,"shapedshim(channel:n, atable:t, steps:n, delay:d) OR shapedshim(channel1:n, atable1:t, channel2:n, atable2:t, steps:n, delay:d) OR shapedshim(channel1:n, atable1:t, channel2:n, atable2:t, channel3:n, atable3:t, steps:n, delay:d)\n");
   ...
   else if(!strcmp(cmd,"spoilerxyz"))           strcpy(syntax,"spoiler(x0:n, y0:n, z0:n, x1:n, y1:n, z1:n, dSpoiler:d, dStab:d)");
   ...
   else if(!strcmp(cmd,"txon"))                 strcpy(syntax,"txon(mode, amp:a/t, phase:p/t [,freq:f/t]) OR txon(ch1, a/t1, p/t1, f/t1, ch2, a/t2, p/t2, f/t2)");
   ...
   else if(!strcmp(cmd,"waltzdec"))             strcpy(syntax,"waltzdec(channel:n, loops:n, amplitudeDec:a, duration90:d, duration180:d, duration270:d, duration360:d, phasePlus:p, phaseMinus:p)");
   else if(!strcmp(cmd,"waltzdecpulse"))        strcpy(syntax,"waltzdecpulse(channel (1/2), dummy_amplitude: a, phase:p/t, duration:d");
   else if(!strcmp(cmd,"waltzdecsetup"))        strcpy(syntax,"waltzdecsetup(channel (1/2), amp:a, [frequency:f]");
   else if(!strcmp(cmd,"wetsuppress"))          strcpy(syntax,"wetsupression(many parameters!)");
...
/******************************************************************************************
  Generate an RF pulse with possible amplitude and ffequency modulation 
  
  chirprf(chirprf(channel (1/2), atable:t, ftable:f, phase:p, table_size:n, table_step_duration:d)
...
/**********************************************************************
              Increment the transmitter frequency (5/7/21)
     ...
     inctxfreq(channel, increment_value)


     channel ..........,...... transmit channel (1/2) default is 1
...
short IncTxFrequency(DLLParameters* par, char *args)
{
   ...
   {
      if((nrArgs = ArgScan(par->itfc,args,2,"channel, increment name","ee","qq",&channel, &incTx)) < 0)
         return(nrArgs);
   }
   else
   {
      Error(par->itfc,"Expecting 1 or 2 arguments");
      return(ERR);
   }
   ...
   if(channel == "1")
   {
      fprintf(fp,"\n;");
      fprintf(fp,"\n;***************************************************************************");
      fprintf(fp,"\n; Increment the channel 1 transmitter frequency");
   ...
   else if(channel == "2")
   {
      fprintf(fp,"\n;");
      fprintf(fp,"\n;***************************************************************************");
      fprintf(fp,"\n; Increment the channel 2 transmitter frequency");
   ...
   {
      Error(par->itfc,"Channel number should be 1 or 2");
      return(ERR);
   }
...
/**********************************************************************
            Modify the receiver frequency (2011-2022)
...
/**********************************************************************
         Modify the transceiver transmitter frequency (3/7/21)
...
// Open the asm file for append
   ...
   if(channel == "1")
   {
      ...
      fprintf(fp,"\n        move    a1,x:TXF10");               // the main Ch-1 Tx frequency location
      fprintf(fp,"\n        move    x:FX%s_1,a1",freq.Str()+1);
      fprintf(fp,"\n        move    a1,x:TXF11");
      fprintf(fp,"\n        rep     #27");
      fprintf(fp,"\n        nop");
   ...
   {
      fprintf(fp,"\n        move    x:FX%s_0,a1",freq.Str()+1); // Copy the new frequency to
      fprintf(fp,"\n        move    a1,x:TXF20");               // the main Ch-2 Tx frequency location
      fprintf(fp,"\n        move    x:FX%s_1,a1",freq.Str()+1);
      fprintf(fp,"\n        move    a1,x:TXF21");
      fprintf(fp,"\n        rep     #27");
      fprintf(fp,"\n        nop");
...
// Get first frequency
   ...
   fprintf(fp,"\n        move    a1,x:TXF10");               // the main Tx frequency location for channel 1
...
// Get second frequency
   ...
   fprintf(fp,"\n        move    a1,x:TXF20");               // the main Tx frequency location for channel 2
...
short RFPulse(DLLParameters* par, char *args)
{
   ...
   if(nrArgs <= 5)
      r = SingleChannelRFPulse(par,args);
   
   else if(nrArgs == 9)
      r = DualChannelRFPulse(par,args);
...
short RFPulseBoost(DLLParameters* par, char *args)
{
   ...
   if((nrArgs = ArgScan(par->itfc,args,2,"channel, boost","ee","qq",&channel,&boost)) < 0)
      return(nrArgs);
   ...
   if(boost[0] == 'n')
      InsertUniqueStringIntoList(boost.Str(),&parList,szList);
   ...
   if(channel == "1")
   {
      if(SelectNumber(par->itfc,fp,boost,"b1",0,1)) return(ERR);
      fprintf(fp,"\n        cmp     #000000,b");
      fprintf(fp,"\n        jeq     LBL%ld",label);
      fprintf(fp,"\n        move    #$000009,a1");
      fprintf(fp,"\n        jmp     LBL%ld",label+1);
      fprintf(fp,"\nLBL%ld    move    #$000049,a1",label);
      fprintf(fp,"\nLBL%ld    move    a1,x:FPGA_DDS1_CFR1",label+1);
      fprintf(fp,"\n        move    #$000000,a1");
   ...
   else if(channel == "2")
   {
      ...
      fprintf(fp,"\n        jmp    LBL%ld",label+1);
      fprintf(fp,"\nLBL%ld  move   #$000049,a1",label);
      fprintf(fp,"\nLBL%ld  move   a1,x:FPGA_DDS2_CFR1",label+1);
      fprintf(fp,"\n        move   #$000000,a1");
   ...
   {
      ErrorMessage("invalid channel number");
   }
...
short DualChannelRFPulse(DLLParameters* par, char *args)
{
   short nrArgs;
...
// Test for valid pulse length
   ...
   fprintf(fp,"\n        jgt    LBL%ld                    ; Long pulses are ok in Spinsolve",label);
   fprintf(fp,"\n        move   #2,b1                     ; Abort code");
   fprintf(fp,"\n        cmp    #24,a                     ; Pulse must be >= 500 ns");
   fprintf(fp,"\n        jlt    ABORT                     ; Otherwise abort"); 
...
/*********************************************************************************************************
 Produce a fast ramped RF pulse using the RAM function in the DDS. Must be used with 
 rampedrfstart and rampedrfend to switch between the normal and ramped profiles
 Duration: pgo + duration + 2us  +  1 us (ramps take 2 us each but effective area is same as square 
 pulse of duration us. Extra padding requires an extra 1us.
**********************************************************************************************************/

short RampedRFPulse(DLLParameters* par, char* args)
{
   short nrArgs;
   CText channel, amplitude, phase, duration;
   short ph;
   char ch;
   long stepSize;

   if ((nrArgs = ArgScan(par->itfc, args, 4, "channel, amplitude, phase, duration", "eeee", "qqqq", &channel, &amplitude, &phase, &duration)) < 0)
      return(nrArgs);
   ...
   if (channel[0] == '1') // Channel 1 code
   {
      fprintf(fp, "\n\n;");
      fprintf(fp, "\n;***************************************************************************");
      fprintf(fp, "\n; Generate a fast ramped RF pulse on channel 1");
      fprintf(fp, "\n;");

      // Get the user defined pulse duration (without ramp)
      fprintf(fp, "\n        clr    a");
      if (SelectDuration(par->itfc, fp, "a1", duration) == ERR) return(ERR);

      // Test for invalid pulse length (< 2 us)
      fprintf(fp, "\n   ; Check for invalid pulse length");
      fprintf(fp, "\n        move   #1,b1                     ; Abort code");
      fprintf(fp, "\n        cmp    #49999,a                  ; Check long pulses >= 1 ms");
      fprintf(fp, "\n        jgt    LBL%ld                    ; Long pulses are ok on Spinsolve", label);
      fprintf(fp, "\n        move   #2,b1                     ; Abort code");
      fprintf(fp, "\n        cmp    #203,a                    ; Pulse must be >= 4070 ns");
      fprintf(fp, "\n        jlt    ABORT                     ; Abort if not");
      fprintf(fp, "\nLBL%ld  nop", label++);
      ...
      fprintf(fp, "\n        move    y:TTL,a1                ; Load the current TTL level");
      if(channel == "1nb")
         fprintf(fp, "\n        or      #$000000,a              ; Internal HPA (Channel 1)");
      else
         fprintf(fp, "\n        or      #$004000,a              ; Internal HPA (Channel 1)");
      fprintf(fp, "\n        move    a1,y:TTL                ; Save new TTL word");
      fprintf(fp, "\n        move    a1,x:FPGA_TTL           ; Update TTL RF remains the same");
      ...
      // Calculate and set step rate values for AD9910 channel 1 profile 1 based on the pulse duration
      fprintf(fp, "\n   ; Modify profile 1 based on pulse duration");
      fprintf(fp, "\n        clr     a");
      fprintf(fp, "\n        move    x:DELAY%s,a1", duration.Str() + 1);
      fprintf(fp, "\n        sub     #101,a                  ; subtract 2us from pulse length for the added ramps");
      fprintf(fp, "\n        lsr     #8,a");
      fprintf(fp, "\n        and     #$FF,a");
      fprintf(fp, "\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp, "\n        move    x:DELAY%s,a1", duration.Str() + 1);
      fprintf(fp, "\n        sub     #101,a                  ; subtract 2us from pulse length for the added ramps");
      fprintf(fp, "\n        lsl     #8,a");
      fprintf(fp, "\n        and     #$FF00,a");
      fprintf(fp, "\n        or      #$5,a                    ; end address 20");
      fprintf(fp, "\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp, "\n        move    #$000004,a1             ; start address 16 ");
      fprintf(fp, "\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp, "\n        move    #$000001,a1             ; Ramp-up mode");
      fprintf(fp, "\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp, "\n        move    #1,r7");
      fprintf(fp, "\n        bsr     wait");
      ...
      // Update channel phase (allow more general phase value?)
      fprintf(fp, "\n; Set phase");
      if (SelectPhaseRampedPulse(par->itfc, fp, phase, channel) == ERR) return(ERR);
      ...
      // Trigger RAM pulse (and process buffered DDS commands)
      fprintf(fp, "\n   ; Trigger RAM pulse (and process buffered DDS commands)");
      ...
      // Load delay into timer
      fprintf(fp, "\n        move    x:DELAY%s,a1", duration.Str() + 1);
      fprintf(fp, "\n        add     #120,a                  ; add ramptime to setup the next pulse before before the current RAM pulse finishes");
      fprintf(fp, "\n        movep   a1,x:A_TCPR2");
      fprintf(fp, "\n        nop");
      ...
      fprintf(fp, "\n        movep   #$200A01,x:A_TCSR2");
      fprintf(fp, "\n        nop");
      fprintf(fp, "\n        jclr    #21,x:A_TCSR2,*         ; Wait for the pulse to end");
      fprintf(fp, "\n   ; End pulse");
      fprintf(fp, "\n        movep   #$200A00,x:A_TCSR2      ; Turn off timer");

      // Switch off the RF
      fprintf(fp, "\n; Switch off channel 1 (TReX and IntRF)");
      fprintf(fp, "\n        nop");
      fprintf(fp, "\n        move    y:TTL,a1                   ; Get the TTL word");
      fprintf(fp, "\n        and     #$FFBFF7,a                 ; Switch off the RF on channel 1");
      fprintf(fp, "\n        move    a1,y:TTL                   ; Update TTL word");
      fprintf(fp, "\n        move    a1,x:FPGA_TTL              ; Update TTL & RF");

      fprintf(fp, "\n        rep     #15"); // Wait 150 ns
      fprintf(fp, "\n        nop"); // Tweak to give an extra 1 us time

      fclose(fp);
   }
   else  if (channel[0] == '2') // Channel 2 code
   {
      fprintf(fp, "\n\n;");
      fprintf(fp, "\n;***************************************************************************");
      fprintf(fp, "\n; Generate a fast ramped RF pulse on channel 2");
      fprintf(fp, "\n;");
      ...
      fprintf(fp, "\n        move    y:TTL,a1                ; Load the current TTL level");
      if (channel == "2nb")
         fprintf(fp, "\n        or      #$000100,a              ; Internal HPA (Channel 2)");
      else
         fprintf(fp, "\n        or      #$000000,a              ; Internal HPA (Channel 2)");
      fprintf(fp, "\n        move    a1,y:TTL                ; Save new TTL word");
      fprintf(fp, "\n        move    a1,x:FPGA_TTL           ; Update TTL RF remains the same");
      ...
      // Load delay into timer
      fprintf(fp, "\n        move    x:DELAY%s,a1", duration.Str() + 1);
      fprintf(fp, "\n        add     #120,a                  ; add 2us to setup the next pulse before before the current RAM pulse finishes");
      fprintf(fp, "\n        movep   a1,x:A_TCPR2");
      fprintf(fp, "\n        nop");

      // Start the timer and wait for the pulse to end
      fprintf(fp, "\n        movep   #$200A01,x:A_TCSR2");
      fprintf(fp, "\n        nop");
      fprintf(fp, "\n        jclr    #21,x:A_TCSR2,*         ; Wait for the pulse to end");
      fprintf(fp, "\n   ; End pulse");
      ...
      // Switch off the RF
      fprintf(fp, "\n        move    y:TTL,a1                   ; Get the TTL word");
      fprintf(fp, "\n        and     #$FFFEFD,a                 ; Switch off the RF on channel 2");
      fprintf(fp, "\n        move    a1,y:TTL                   ; Update TTL word");
      fprintf(fp, "\n        move    a1,x:FPGA_TTL              ; Update TTL & RF");
...
//{
//   short nrArgs;
//   CText channel,amp,phase,duration,stepExp="3";
//   long Frequency;
//   short ph;
//   char ch;
//   long stepSize;
//
//   if((nrArgs = ArgScan(par->itfc,args,4,"channel,amp,phase,duration,stepsExponent","eeeee","qqqqq",&channel,&amp,&phase,&duration,&stepExp)) < 0)
//      return(nrArgs);
//
//   if(!parList)
//   {
...
//      fprintf(fp,"\n        movep   #$200A01,x:A_TCSR2");   
//      
//   // Update channel amplitude (must be divisible my number of steps)
//      fprintf(fp,"\n; Set channel amplitude");
//      long nrSteps;
//      if(SelectAmplitudeRamped(par->itfc ,fp, amp, stepExp, channel, nrSteps) == ERR) return(ERR);
//   // Update channel phase (allow more general phase value?)
//      fprintf(fp,"\n; Set phase");
//      if(SelectPhase(par->itfc ,fp, phase,channel) == ERR) return(ERR);
//
...
//
//   // Save the duration to the timer
//      fprintf(fp,"\n; Start pulse");
//      if(SelectDuration(par->itfc ,fp, "a1", duration) == ERR) return(ERR);
//      fprintf(fp,"\n        sub     #11,a");
//      fprintf(fp,"\n        move    a1,x:A_TCPR2");
...
//         fprintf(fp,"\n        rep     #9                     ; Wait for 1 us");
//         fprintf(fp,"\n        nop");
//         fprintf(fp,"\n        move    b1,a1                  ; Load the current amplitude");
//
//         fprintf(fp,"\n        move    #%ld,r1                  ; Repeat n-2 times",nrSteps-2);
//         fprintf(fp,"\n        do      r1,LBL%ld              ; Repeat code until Loop end",label);
//            fprintf(fp,"\n        add     b,a                    ; Add step size to amplitude");
//            fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0    ; Update amplitude");
//            fprintf(fp,"\n        rep     #84                    ; Wait for 1 us");
//         fprintf(fp,"\n        nop");
//
//         fprintf(fp,"\nLBL%ld  nop",label++);
//
...
//         fprintf(fp,"\n        do      r1,LBL%ld              ; Repeat code until Loop end",label);
//            fprintf(fp,"\n        add     b,a                    ; Add step size to amplitude");
//            fprintf(fp,"\n        move    a1,x:FPGA_DDS2_Pro0    ; Update amplitude");
//            fprintf(fp,"\n        rep     #84                    ; Wait for 1 us");
//         fprintf(fp,"\n        nop");
//
//         fprintf(fp,"\nLBL%ld  nop",label++);
...
//      }
//
//     // End center part of RF pulse
//      fprintf(fp,"\n; End pulse");
//      fprintf(fp,"\n        movep   #$200A00,x:A_TCSR2      ; Turn off timer");
//
//      // Add the ramp down here
//
//       if(channel == "1" ||  channel == "1nb")
//       {
//         fprintf(fp,"\n        move    #%ld,r1                  ; Repeat n-2 times",nrSteps-2);
//         fprintf(fp,"\n        do      r1,LBL%ld              ; Repeat code until Loop end",label);
//            fprintf(fp,"\n        sub     b,a                    ; Subtract step size from amplitude");
//            fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0    ; Update amplitude");
//            fprintf(fp,"\n        rep     #84                    ; Wait for 1 us");
//            fprintf(fp,"\n        nop");
//         fprintf(fp,"\nLBL%ld  nop",label++);
...
//
//      fprintf(fp,"\n        clr    a");
//      fprintf(fp,"\n        move     y:TTL,a1              ; Get TTL & RF value");
//
//      if(channel == "1")
//         fprintf(fp,"\n        and     #$FFBFF7,a              ; Switch off channel 1");
//      else if(channel == "2")
...
/*********************************************************************************************************
 Set up a fast-ramped pulse for the waltz decoupling. This needs to be the first command in the sequence
**********************************************************************************************************/

short WaltzDecSetUp(DLLParameters* par,char *args)
{
   short nrArgs;
   CText channel,amplitude,frequency;

   if((nrArgs = ArgScan(par->itfc,args,2,"channel,amplitude,[frequency]","eee","qqq",&channel,&amplitude,&frequency)) < 0)
      return(nrArgs);
   ...
   if(amplitude[0] == 'a')
      InsertUniqueStringIntoList(amplitude.Str(),&parList,szList);
   if(frequency[0] == 'f')
      InsertUniqueStringIntoList(frequency.Str(),&parList,szList);
   ...
   if(channel == "1")
   {
      fprintf(fp,"\n\n;");
      fprintf(fp,"\n;***************************************************************************");
      fprintf(fp,"\n; Generate a ramped RF pulse on channel 1 - setup code");
      fprintf(fp,"\n;"); 

      // Set values for AD9910 channel 1 profile 0
      // Set up start and end address to write the whole data to RAM
      fprintf(fp,"\n        move    #$000000,a1"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      fprintf(fp,"\n        move    #$002009,a1             ; step rate 32 (128ns), endAddr 36"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      fprintf(fp,"\n        move    #$000000,a1             ; start addr 0"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      fprintf(fp,"\n        move    #$000001,a1             ; Ramp-up mode"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      fprintf(fp,"\n        move    #3,r7"); 
      ...
      // Set channel 1 amplitude
      fprintf(fp,"\n; Set channel 1 amplitude");
      fprintf(fp,"\n        move    x:TXA%s,b1              ; Get amplitude",amplitude.Str()+1);
      fprintf(fp,"\n        lsr     #2,b                    ; Divide amplitude by 16 (and left shift x2 for MSB alignment)"); 
      fprintf(fp,"\n        move    b,r2                    ; store 1/16 of amplitude in r2"); 

      fprintf(fp,"\n; Ramp up 15");
      fprintf(fp,"\n        move    #$000000,a1");  // Initalise with zero amplitude
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_RR");
      fprintf(fp,"\n        move    #$000000,b1"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_RR");
      fprintf(fp,"\n        move    #2,r7"); 
      fprintf(fp,"\n        bsr     svwait"); 
      fprintf(fp,"\n        move    #15,r4"); 
      fprintf(fp,"\n        do      r4,LBL%ld               ; ramp up loop",label);  // Do the remaining  15 steps
      fprintf(fp,"\n        move    r2,b1"); 
      fprintf(fp,"\n        add     b,a"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_RR");
      fprintf(fp,"\n        clr     b"); 
      fprintf(fp,"\n        move    b1,x:FPGA_DDS1_RR");
      fprintf(fp,"\n        move    #2,r7"); 
      fprintf(fp,"\n        bsr     svwait"); 
      fprintf(fp,"\nLBL%ld  nop",label++);
      ...
      fprintf(fp,"\n; Hold the last value in the ramp");
      fprintf(fp,"\n        move    x:TXA%s,a1              ; Get amplitude",amplitude.Str()+1);
      fprintf(fp,"\n        lsl     #2,a                    ; Left shift x2 for MSB alignment"); 
      fprintf(fp,"\n        move    #5,r4                   ; store 1/16 of amplitude in r2"); 
      fprintf(fp,"\n        do      r4,LBL%ld               ; constant value loop",label); 
      fprintf(fp,"\n        nop"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_RR");
      fprintf(fp,"\n        clr     b"); 
      fprintf(fp,"\n        move    b1,x:FPGA_DDS1_RR");
      ...
      fprintf(fp,"\n; Ramp down 16");
      fprintf(fp,"\n        move    #16,r4"); 
      fprintf(fp,"\n        do      r4,LBL%ld               ; ramp down loop",label); 
      fprintf(fp,"\n        move    r2,b1"); 
      fprintf(fp,"\n        sub     b,a"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_RR");
      fprintf(fp,"\n        clr     b"); 
      ...
      fprintf(fp,"\n        move    #$000000,a1"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      fprintf(fp,"\n        move    #$002003,a1             ; step rate 32 (128ns), endAddr 15"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      fprintf(fp,"\n        move    #$00c000,a1             ; start addr 0"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      fprintf(fp,"\n        move    #$000001,a1             ; Ramp-up mode"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      fprintf(fp,"\n        move    #3,r7"); 
      ...
      fprintf(fp,"\n        move    #$000013,a1"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp,"\n        move    #$008805,a1             ; step rate for 100us pulse (0x1388=0d5000)*20ns, endAddr 20"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp,"\n        move    #$000004,a1             ; start addr 16"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp,"\n        move    #$000001,a1             ; Ramp-up mode"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp,"\n        move    #3,r7"); 
      ...
      fprintf(fp,"\n; Set profile 2");
      fprintf(fp,"\n        move    #$000000,a1"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro2");
      fprintf(fp,"\n        move    #$002009,a1             ; step rate 32 (128ns), endAddr 36"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro2");
      fprintf(fp,"\n        move    #$000005,a1             ; start addr 21"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro2");
      fprintf(fp,"\n        move    #$004001,a1             ; Ramp-up mode"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro2");
      fprintf(fp,"\n        move    #3,r7"); 
      fprintf(fp,"\n        bsr     wait");

      fprintf(fp,"\n; Set Frequency");
      if(nrArgs == 3) // User defined
      {
         if (frequency[0] == 'f')
         {
            fprintf(fp, "\n        move    x:FX%s_0,a1", frequency.Str() + 1);
            fprintf(fp, "\n        move    a1,x:FPGA_DDS1_FTW");
            fprintf(fp, "\n        move    x:FX%s_1,a1", frequency.Str() + 1);
            fprintf(fp, "\n        move    a1,x:FPGA_DDS1_FTW");
            fprintf(fp, "\n        move    #2,r7");
            fprintf(fp, "\n        bsr     wait");
         }
         else
         {
            ...
            if (sscanf(frequency.Str(), "%lf", &fFreq) == 1) // Fixed frequency
            {
               if (fFreq < 1 || fFreq > 150)
               {
                  Error(par->itfc, "invalid frequency '%lg' [1 ... 150]", fFreq);
                  fclose(fp);
                  return(ERR);
               }
               __int64 DDSFword = (__int64)((fFreq * 4294967296.0L) / 1000.0L + 0.5);
      ...
      fprintf(fp, "\n        rep     #130"); // Wait 1300 ns
      fprintf(fp, "\n        nop"); // Tweak to give a total of 50 us duration
   ...
   else if(channel == "2")
   {
      fprintf(fp,"\n\n;");
      fprintf(fp,"\n;***************************************************************************");
      fprintf(fp, "\n; Generate a ramped RF pulse on channel 2 - setup code");
      fprintf(fp,"\n;");
      ...
      fprintf(fp,"\n; Ramp up 15");
      fprintf(fp,"\n        move    #$000000,a1"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS2_RR");
      fprintf(fp,"\n        move    #$000000,b1"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS2_RR");
      fprintf(fp,"\n        move    #2,r7"); 
      fprintf(fp,"\n        bsr     svwait"); 
      fprintf(fp,"\n        move    #15,r4"); 
      fprintf(fp,"\n        do      r4,LBL%ld               ; ramp up loop",label); 
      fprintf(fp,"\n        move    r2,b1"); 
      fprintf(fp,"\n        add     b,a"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS2_RR");
      fprintf(fp,"\n        clr     b"); 
      fprintf(fp,"\n        move    b1,x:FPGA_DDS2_RR");
      fprintf(fp,"\n        move    #2,r7"); 
      fprintf(fp,"\n        bsr     svwait"); 
      fprintf(fp,"\nLBL%ld  nop",label++);
   ...
   {
      fclose(fp);
      Error(par->itfc,"unknown RF ramp-pulse channel number");
      return(ERR);
   }
...
/*********************************************************************************************************
 The high level interface to the WET suppression sequence
**********************************************************************************************************/

short WetSuppression(DLLParameters* par, char* args)
{
   short nrArgs;
   CText tAmpPhase1, tAmpPhase2, tAmpPhase3, tAmpPhase4;
   CText phase1, phase2, phase3;
   CText tableSz, tableStep;
   CText nxSpoil1, nySpoil1, nzSpoil1;
   CText nxSpoil2, nySpoil2, nzSpoil2;
   CText nxSpoil3, nySpoil3, nzSpoil3;
   CText nxSpoil4, nySpoil4, nzSpoil4;
   CText nxShim, nyShim, nzShim, nzSlice, dSpoil, dRecov;
   CText tableAmpPhaseRF, tableSzRF, tableStepRF;

   // Get the (many!) arguments
   if ((nrArgs = ArgScan(par->itfc, args, 30, "tableAmpPhase(1-4), p1, p2, tableSz, tableStep, nx/y/zSpoil1, nx/y/zSpoil2, nx/y/zSpoil3, nx/y/xzSpoil4, nxShim, nyShim, nzShim, nzSlice, dSpoil, dRecov, tableAmpPhaseRF, phase3, tableSzRF, tableStepRF",
      "eeeeeeeeeeeeeeeeeeeeeeeeeeeeee", "qqqqqqqqqqqqqqqqqqqqqqqqqqqqqq",
      &tAmpPhase1, &tAmpPhase2, &tAmpPhase3, &tAmpPhase4, &phase1, &phase2, &tableSz, &tableStep,
      &nxSpoil1, &nySpoil1, &nzSpoil1, &nxSpoil2, &nySpoil2, &nzSpoil2, &nxSpoil3, &nySpoil3, &nzSpoil3, &nxSpoil4, &nySpoil4, &nzSpoil4,
      &nxShim, &nyShim, &nzShim, &nzSlice, &dSpoil, &dRecov, &tableAmpPhaseRF, &phase3, &tableSzRF, &tableStepRF)) < 0)
      return(nrArgs);
   ...
   // Saturate the 1H peaks using a soft pulse
   arg.Format("%s,%s,%s,%s,%s", tAmpPhase1.Str(), phase1.Str(), phase2.Str(), tableSz.Str(), tableStep.Str());
   DualShapedRFPulse2(par, arg.Str());
   arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", nxSpoil1.Str(), nySpoil1.Str(), nzSpoil1.Str(), nxShim.Str(), nyShim.Str(), nzShim.Str(), dSpoil.Str(), dRecov.Str());
   SpoilerXYZ(par, arg.Str());

   arg.Format("%s,%s,%s,%s,%s", tAmpPhase2.Str(), phase1.Str(), phase2.Str(), tableSz.Str(), tableStep.Str());
   DualShapedRFPulse2(par, arg.Str());
   arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", nxSpoil2.Str(), nySpoil2.Str(), nzSpoil2.Str(), nxShim.Str(), nyShim.Str(), nzShim.Str(), dSpoil.Str(), dRecov.Str());
   SpoilerXYZ(par, arg.Str());

   arg.Format("%s,%s,%s,%s,%s", tAmpPhase3.Str(), phase1.Str(), phase2.Str(), tableSz.Str(), tableStep.Str());
   DualShapedRFPulse2(par, arg.Str());
   arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", nxSpoil3.Str(), nySpoil3.Str(), nzSpoil3.Str(), nxShim.Str(), nyShim.Str(), nzShim.Str(), dSpoil.Str(), dRecov.Str());
   SpoilerXYZ(par, arg.Str());

   arg.Format("%s,%s,%s,%s,%s", tAmpPhase4.Str(), phase1.Str(), phase2.Str(), tableSz.Str(), tableStep.Str());
   DualShapedRFPulse2(par, arg.Str());
   arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", nxSpoil4.Str(), nySpoil4.Str(), nzSpoil4.Str(), nxShim.Str(), nyShim.Str(), nzShim.Str(), dSpoil.Str(), dRecov.Str());
   SpoilerXYZ(par, arg.Str());

   //  Slice selection
   arg.Format("0,%s,%s,200, 4", nzShim.Str(), nzSlice.Str());
   ShimRamp(par, arg.Str());
   Delay(par, "100");

   arg.Format("1,%s,%s,%s,%s", tableAmpPhaseRF.Str(), phase3.Str(), tableSzRF.Str(), tableStepRF.Str());
   ShapedRFPulse2(par, arg.Str());
   Delay(par, "100");

   arg.Format("0,%s,%s,200, 4", nzSlice.Str(), nzShim.Str()); // z
   ShimRamp(par, arg.Str());

   arg.Format("2,%s,%s,200, 4", nyShim.Str(), nySpoil4.Str()); // y
   ShimRamp(par, arg.Str());
   Delay(par, "5000");

   arg.Format("2,%s,%s,200, 4", nySpoil4.Str(), nyShim.Str()); // y
   ShimRamp(par, arg.Str());
   Delay(par, "2000");
   //  End Slice selection   
...
//
//short WetSuppressionOrig(DLLParameters* par, char* args)
//{
//    short nrArgs;
//    CText tAmp1, tAmp2, tAmp3, tAmp4;
//    CText tPhase1, tPhase2, tPhase3, tPhase4;
//    CText phase1, phase2, phase3;
//    CText freq1, freq2; 
//    CText tableSz, tableStep;
//    CText nxSpoil1, nySpoil1, nzSpoil1;
...
//    CText nxSpoil4, nySpoil4, nzSpoil4;
//    CText nxShim, nyShim, nzShim, nzSlice, dSpoil, dRecov;
//    CText tableAmpRF, tablePhaseRF, tableSzRF, tableStepRF;
//
//    // Get the (many!) arguments
//    if ((nrArgs = ArgScan(par->itfc, args, 37, "tableAmp(1-4), tablePhase(1-4), p1, p2, f1, f2, tableSz, tableStep, nx/y/zSpoil1, nx/y/zSpoil2, nx/y/zSpoil3, nx/y/xzSpoil4, nxShim, nyShim, nzShim, nzSlice, dSpoil, dRecov, tableAmpRF, tablePhaseRF, phase3, tableSzRF, tableStepRF",
//                                               "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee", "qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq",
//                                               &tAmp1, &tAmp2, &tAmp3, &tAmp4, &tPhase1, &tPhase2, &tPhase3, &tPhase4, &phase1, &phase2, &freq1, &freq2, &tableSz, &tableStep,
//                                               &nxSpoil1, &nySpoil1, &nzSpoil1, &nxSpoil2, &nySpoil2, &nzSpoil2, &nxSpoil3, &nySpoil3, &nzSpoil3, &nxSpoil4, &nySpoil4, &nzSpoil4,
//                                               &nxShim, &nyShim, &nzShim, &nzSlice, &dSpoil, &dRecov, &tableAmpRF, &tablePhaseRF, &phase3, &tableSzRF, &tableStepRF)) < 0)
//        return(nrArgs);
//
//    if (!parList)
//    {
...
//
//    // Saturate the 1H peaks using a soft pulse
//    arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", tAmp1.Str(), tPhase1.Str(), phase1.Str(), phase2.Str(), freq1.Str(), freq2.Str(), tableSz.Str(), tableStep.Str());
//    ShapedRFPulse(par, arg.Str());
//    arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", nxSpoil1.Str(), nySpoil1.Str(), nzSpoil1.Str(), nxShim.Str(), nyShim.Str(), nzShim.Str(), dSpoil.Str(), dRecov.Str());
//    SpoilerXYZ(par, arg.Str());
//
//    arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", tAmp2.Str(), tPhase2.Str(), phase1.Str(), phase2.Str(), freq1.Str(), freq2.Str(), tableSz.Str(), tableStep.Str());
//    ShapedRFPulse(par, arg.Str());
//    arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", nxSpoil2.Str(), nySpoil2.Str(), nzSpoil2.Str(), nxShim.Str(), nyShim.Str(), nzShim.Str(), dSpoil.Str(), dRecov.Str());
//    SpoilerXYZ(par, arg.Str());
//
//    arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", tAmp3.Str(), tPhase3.Str(), phase1.Str(), phase2.Str(), freq1.Str(), freq2.Str(), tableSz.Str(), tableStep.Str());
//    ShapedRFPulse(par, arg.Str());
//    arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", nxSpoil3.Str(), nySpoil3.Str(), nzSpoil3.Str(), nxShim.Str(), nyShim.Str(), nzShim.Str(), dSpoil.Str(), dRecov.Str());
//    SpoilerXYZ(par, arg.Str());
//
//    arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", tAmp4.Str(), tPhase4.Str(), phase1.Str(), phase2.Str(), freq1.Str(), freq2.Str(), tableSz.Str(), tableStep.Str());
//    ShapedRFPulse(par, arg.Str());
//    arg.Format("%s,%s,%s,%s,%s,%s,%s,%s", nxSpoil4.Str(), nySpoil4.Str(), nzSpoil4.Str(), nxShim.Str(), nyShim.Str(), nzShim.Str(), dSpoil.Str(), dRecov.Str());
//    SpoilerXYZ(par, arg.Str());
//
////  Slice selection
//    arg.Format("0,%s,%s,200, 4", nzShim.Str(), nzSlice.Str());
//    RampedShim16(par, arg.Str());
//    MakeADelay(par, "100");
//
//    arg.Format("1,%s,%s,%s,%s,%s", tableAmpRF.Str(), tablePhaseRF.Str(), phase3.Str(), tableSzRF.Str(), tableStepRF.Str());
//    ShapedRFPulse(par, arg.Str());
//    MakeADelay(par, "100");
//
//    arg.Format("0,%s,%s,200, 4", nzSlice.Str(), nzShim.Str()); // z
...
/*********************************************************************************************************
 The high level interface to the Spoiler sequence
**********************************************************************************************************/

short SpoilerXYZ(DLLParameters* par, char* args)
{
   short nrArgs;
   CText x0, y0, z0, x1, y1, z1, dSpoil, dStab;
   CText arg;

   if ((nrArgs = ArgScan(par->itfc, args, 8, "x0, y0, z0, x1, y1, z1, duration, dStab", "eeeeeeee", "qqqqqqqq",
                                             &x0, &y0, &z0, &x1, &y1, &z1, &dSpoil, &dStab)) < 0)
      return(nrArgs);

   arg.Format("1,%s", x0.Str()); SwitchOnShim16(par, arg.Str());
   arg.Format("2,%s", y0.Str()); SwitchOnShim16(par, arg.Str());
   arg.Format("0,%s", z0.Str()); SwitchOnShim16(par, arg.Str());
   arg.Format("%s", dSpoil);     Delay(par, arg.Str());

   arg.Format("1,%s", x1.Str()); SwitchOnShim16(par, arg.Str());
   arg.Format("2,%s", y1.Str()); SwitchOnShim16(par, arg.Str());
   arg.Format("0,%s", z1.Str()); SwitchOnShim16(par, arg.Str());
   arg.Format("%s", dStab);      Delay(par, arg.Str());
...
/*********************************************************************************************************
 The high level interface to the Waltz16 decoupling sequence
**********************************************************************************************************/

void WaltzQ(DLLParameters* par, CText &channel,CText &loops,CText &amplitude,CText &duration90,CText &duration180,CText &duration270,CText &duration360,CText &phasePlus,CText &phaseMinus);

short WaltzDecoupling(DLLParameters* par, char *args)
{
   short nrArgs;
   CText channel, amplitude;
   CText duration90,duration180,duration270,duration360;
   CText phasePlus,phaseMinus, loops;
   CText arg;

   if((nrArgs = ArgScan(par->itfc,args,9,"channel,loops,amplitude, d90, d180, d270, d360, p2, p3","eeeeeeeee","qqqqqqqqq",
                                         &channel,&loops,&amplitude,&duration90,&duration180,&duration270,&duration360,
                                         &phasePlus,&phaseMinus)) < 0)
      return(nrArgs);
   ...
   arg.Format("%s,%s",channel.Str(),duration270.Str());      WaltzStart(par, arg.Str());
   arg.Format("%s,%s","lWaltzDec",loops.Str());            LoopStart(par,arg.Str());
//   arg.Format("%s,%s","lWaltzDec2",4);                      LoopStart(par,arg.Str());

   WaltzQ(par,channel,loops,amplitude,duration90,duration180,duration270,duration360,phasePlus,phaseMinus);
   WaltzQ(par,channel,loops,amplitude,duration90,duration180,duration270,duration360,phaseMinus,phasePlus);
   WaltzQ(par,channel,loops,amplitude,duration90,duration180,duration270,duration360,phaseMinus,phasePlus);
   WaltzQ(par,channel,loops,amplitude,duration90,duration180,duration270,duration360,phasePlus,phaseMinus);

   //---------
//   arg = "lWaltzDec2";                                     LoopEnd(par,arg.Str());
   arg = "lWaltzDec";                                     LoopEnd(par,arg.Str());
   arg = channel.Str();                                   WaltzEnd(par, arg.Str());
...
void WaltzQ(DLLParameters* par, CText &channel,CText &loops,CText &amplitude,CText &duration90,CText &duration180,CText &duration270,CText &duration360,CText &phasePlus,CText &phaseMinus)
{
   ...
   //--------- Q --------------------
   arg.Format("%s,%s,%s,%s",channel.Str(),amplitude.Str(),
              phaseMinus.Str(),duration270.Str());          WaltzDecPulse(par,arg.Str());
   arg = "4.4";                                             Delay(par,arg.Str());

   arg.Format("%s,%s,%s,%s",channel.Str(),amplitude.Str(),
              phasePlus.Str(),duration360.Str());           WaltzDecPulse(par,arg.Str());
   arg = "4.4";                                             Delay(par,arg.Str());

   arg.Format("%s,%s,%s,%s",channel.Str(),amplitude.Str(),
              phaseMinus.Str(),duration180.Str());          WaltzDecPulse(par,arg.Str());
   arg = "4.4";                                             Delay(par,arg.Str());
   //--------- 2
   arg.Format("%s,%s,%s,%s",channel.Str(),amplitude.Str(),
              phasePlus.Str(),duration270.Str());           WaltzDecPulse(par,arg.Str());
   arg = "4.4";                                             Delay(par,arg.Str());

   arg.Format("%s,%s,%s,%s",channel.Str(),amplitude.Str(),
              phaseMinus.Str(),duration90.Str());           WaltzDecPulse(par,arg.Str());
   arg = "4.4";                                             Delay(par,arg.Str());

   arg.Format("%s,%s,%s,%s",channel.Str(),amplitude.Str(),
              phasePlus.Str(),duration180.Str());           WaltzDecPulse(par,arg.Str());
   arg = "4.4";                                             Delay(par,arg.Str());
   //--------- 3
   arg.Format("%s,%s,%s,%s",channel.Str(),amplitude.Str(),
              phaseMinus.Str(),duration360.Str());          WaltzDecPulse(par,arg.Str());
   arg = "4.4";                                             Delay(par,arg.Str());
...
{
   short nrArgs;
   CText channel,duration;

   if((nrArgs = ArgScan(par->itfc,args,2,"channel,duration","ee","qq",&channel,&duration)) < 0)
      return(nrArgs);
   ...
   if(channel == "1")
   {
      fprintf(fp,"\n\n;");
      fprintf(fp,"\n;***************************************************************************");
      fprintf(fp,"\n; Setup code for the WALTZ16 sequence");
      fprintf(fp,"\n;"); 

      fprintf(fp,"\n        move    y:TTL,a1                ; Load the current TTL word");
      fprintf(fp,"\n        or      #$004000,a              ; TX Enable (IntRF 1H)");
      fprintf(fp,"\n        move    a1,y:TTL                ; Update TTL word");
      fprintf(fp,"\n        move    a1,x:FPGA_TTL           ; Update TTL & RF");
      ...
      fprintf(fp,"\n; Set and update first pulse step rate");
      fprintf(fp,"\n        clr     a");
      fprintf(fp,"\n        move    x:DELAY%s,a1",duration.Str()+1);
      fprintf(fp,"\n        sub     #101,a                  ; subtract 2.02us from pulse length for the added ramps");
      fprintf(fp,"\n        lsr     #8,a                     ; get high bytes");
      fprintf(fp,"\n        and     #$FF,a");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp,"\n        move    x:DELAY%s,a1",duration.Str()+1);
      fprintf(fp,"\n        sub     #101,a                  ; subtract 2.02us from pulse length for the added ramps");
      fprintf(fp,"\n        lsl     #8,a                     ; shift low byte to high byte");
      fprintf(fp,"\n        and     #$FF00,a");
      fprintf(fp,"\n        or      #$5,a                    ; end address 20");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp,"\n        move    #$000004,a1             ; start address 16");
      ...
      fprintf(fp,"\n; disable Auto IO Update");
      fprintf(fp,"\n        move    #$000000,a1             ;#$000400 to enable Auto IOU");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS_CR");
      fprintf(fp,"\n        move    y:TTL,a1                ; Load the current TTL word");
      fprintf(fp,"\n        or      #$000008,a              ; Channel 1 RF on");
      fprintf(fp,"\n        move    a1,y:TTL                ; Load the current TTL word");
      fprintf(fp,"\n        move    a1,x:FPGA_TTL           ; Update TTL & RF");
      fprintf(fp,"\n        move    #$00c045,a1                 ; CFR1 32-16  RAM enabled, RAM destination = AMP, Internal Profile Control Mode (Pro0-Pro2 then halt), Cosine");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_CFR1");
...
short WaltzEnd(DLLParameters* par,char *args)
{
   ...
   if((nrArgs = ArgScan(par->itfc,args,1,"channel","e","q",&channel)) < 0)
      return(nrArgs);
   ...
   if(channel == "1")
   {
      fprintf(fp,"\n\n;");
      fprintf(fp,"\n;***************************************************************************");
      fprintf(fp,"\n; End code for the WALTZ16 sequence");
      fprintf(fp,"\n;"); 
      ...
      fprintf(fp,"\n        move    #$000400,a1");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS_CR");
      fprintf(fp,"\n        move    #$004045,a1                 ; CFR1 32-16  RAM disabled, RAM destination = AMP, Internal Profile Control Mode (Pro0-Pro2 then halt), Cosine");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_CFR1");
      fprintf(fp,"\n        move    #$000000,a1                 ; CFR1 15-0");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_CFR1");
      fprintf(fp,"\n        move    #2,r7");
   ...
   else if(channel == "2")
   {
      ...
      fprintf(fp,"\n; Switch off channel 2 (TReX and IntRF)");
...
/*********************************************************************************************************
 Switch to using the ramp pulse DDS profile
 Duration : 5 us
**********************************************************************************************************/
...
short RampedRFPulseStart(DLLParameters* par, char* args)
{
   ...
   if (channel == "1")
   {
      fprintf(fp, "\n\n;");
      fprintf(fp, "\n;***************************************************************************");
      fprintf(fp, "\n; Set up the profile for the RampedRFPulse command (ch1)");
      fprintf(fp, "\n;");
      ...
      fprintf(fp, "\n        rep     #201"); // Wait 2010 ns
      fprintf(fp, "\n        nop"); // Tweak to give a 5 us time
      fclose(fp);
   }
   else if (channel == "2")
   {
      fprintf(fp, "\n\n;");
      fprintf(fp, "\n;***************************************************************************");
      fprintf(fp, "\n; Set up the profile for the RampedRFPulse command (ch2)");
      fprintf(fp, "\n;");
...
/*********************************************************************************************************
 Switch to using the normal DDS profile after ramped pulses used
 Duration : 5 us
**********************************************************************************************************/
...
short RampedRFPulseEnd(DLLParameters* par, char* args)
{
   ...
   if (channel == "1")
   {
      fprintf(fp, "\n\n;");
      fprintf(fp, "\n;***************************************************************************");
      fprintf(fp, "\n; Restore the ch1 DDS profile to normal after a series of RampedRFPulse commands (ch1)");
      fprintf(fp, "\n;");
   ...
   else if (channel == "2")
   {
      fprintf(fp, "\n\n;");
      fprintf(fp, "\n;***************************************************************************");
      fprintf(fp, "\n; Restore the ch2 DDS profile to normal after a series of RampedRFPulse commands");
      fprintf(fp, "\n;");
...
// A place holder - will implement

short RampedPulse(DLLParameters* par, char* args)
{
   CText chName = " ", ampName = " ", phName = " ", durationName = " ", nrStepsName = " ";
   short nrArgs;

   if ((nrArgs = ArgScan(par->itfc, args, 4, "channel, amplitude, phase, stepSize", "cccc", "tttt", &chName, &ampName, &phName, &durationName)) < 0)
      return(nrArgs);
...
/*********************************************************************************************************
 Generate a fast-ramped pulse for the Waltz decoupling sequence. (Note that the setup code must be present)
**********************************************************************************************************/

short WaltzDecPulse(DLLParameters* par,char *args)
{
   short nrArgs;
   CText channel,amplitude,phase,duration;
   short ph;
   ...
   // Load delay into timer
      fprintf(fp,"\n        move    x:DELAY%s,a1",duration.Str()+1);
    //  fprintf(fp,"\n        add     #50,a                  ; subtract 2us to setup the next pulse before before the current RAM pulse finishes");
      fprintf(fp,"\n        sub     #50,a                  ; subtract 2us to setup the next pulse before before the current RAM pulse finishes");
      fprintf(fp,"\n        movep   a1,x:A_TCPR2");
      fprintf(fp,"\n        nop");
     // Start the timer
      fprintf(fp,"\n        movep   #$200A01,x:A_TCSR2");   
   ...
   // Calculate and set step rate values for AD9910 channel 1 profile 1 based on the pulse duration
    ...
    // Load delay into timer
      ...
      fprintf(fp,"\n        sub     #50,a                  ; subtract 2us to setup the next pulse before before the current RAM pulse finishes"); // Was 100
      fprintf(fp,"\n        movep   a1,x:A_TCPR2");
      fprintf(fp,"\n        nop");
...
/**********************************************************************
     Generate a shaped rf pulse on one or both Tx channels
**********************************************************************/
...
short ShapedRFPulse(DLLParameters* par, char *args)
{
   ...
   if(nrArgs == 6 || nrArgs == 7)
   {
      r = ShapedRFSingleChannel(par, args);
   }
   else if (nrArgs == 5)
   {
      r = ShapedRFSingleChannel(par, args);
   ...
   {
      r = ShapedRFDualChannel(par, args);
   }
   return(r);
}
...
/**********************************************************************
     ...
     shapedrf(channel, amplitude, amplitudeTable, phaseTable, phase, tableSize, tableStep)

     channel .......... transmit channels (1/2)
...
{
   short nrArgs;
   CText channel,dur,atable,ptable,size,phase,duration,freq;
   long Duration,Phase,Size;
   float fDuration;
   const int pgoOffset =  5;
   const int firstDelay = 198;
   ...
   if((nrArgs = ArgScan(par->itfc,args,6,"channel, atable, stable, phase, table_size, table_step [,frequency]","eeeeeee","qqqqqqq",&channel,&atable,&ptable,&phase,&size,&duration,&freq)) < 0)
      return(nrArgs);
   if(!parList)
   {
      ErrorMessage("Pulse sequence not initialised");
   ...
   // Add phase shift for phase cycling
      ...
      if(nrArgs == 7) // Set the frequency if supplied
      {
         fprintf(fp,"\n        move    x:FX%s_0,a1",freq.Str()+1);
         fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0     ; Set correct frequency");
         fprintf(fp,"\n        move    x:FX%s_1,a1",freq.Str()+1);
   ...
   // Add phase shift for phase cycling
      ...
      {
         fprintf(fp,"\n        rep     #17"); // Adjust delay to match frequency setting commands above
         fprintf(fp,"\n        nop");
      }
      ...
      if(nrArgs == 7) 
      {
         fprintf(fp,"\n        rep     #59"); // Adjust delay if the frequency has been set
         fprintf(fp,"\n        nop");
      }
   ...
   // Add phase shift for phase cycling
      ...
      fprintf(fp,"\n        nop");
       ...
       if(nrArgs == 7) 
      {
         fprintf(fp,"\n        rep     #57"); // Adjust delay if the frequency has been set
         fprintf(fp,"\n        nop");
      }
...
/*
* Dummy code for FX3 support (single channel amplitude/frequency modulated pulse)
*/
...
short ChirpedRF2(DLLParameters* par, char* args)
{
   ...
   if ((nrArgs = ArgScan(par->itfc, args, 5, "channel, aftable, phase, table_size, table_step", "eeeee", "qqqqq", &channel, &atable, &phase, &size, &duration)) < 0)
      return(nrArgs);
   if (!parList)
   {
      ErrorMessage("Pulse sequence not initialised");
   ...
   TextMessage("ChirpedRF2 not implemented yet on DSP\n");
...
short ShapedRFPulse2(DLLParameters* par, char* args)
{
   ...
   else if (channel == "2") // Channel 2 pulse
   {
      fprintf(fp, "\n\n;");
      fprintf(fp, "\n;***************************************************************************");
...
{
   short nrArgs;
   CText aptables, size, phase1, phase2, duration, freq;

   if ((nrArgs = ArgScan(par->itfc, args, 5, "atables, phase1, phase2, table_size, table_step", "eeeee", "qqqqq", &aptables, &phase1, &phase2, &size, &duration)) < 0)
      return(nrArgs);
   if (!parList)
   {
      ErrorMessage("Pulse sequence not initialised");
   ...
   TextMessage("DualShapedRFPulse1 not implemented yet on DSP\n");
...
short DualShapedRFPulse2(DLLParameters* par, char* args)
{
   ...
   if ((nrArgs = ArgScan(par->itfc, args, 5, "aptable, phase1, phase2, table_size, table_step", "eeeee", "qqqqq", &aptable, &phase1, &phase2, &size, &duration)) < 0)
      return(nrArgs);
   if (!parList)
   {
      ErrorMessage("Pulse sequence not initialised");
...
/**********************************************************************
     Generate a dual channel shaped RF pulse (14/8/20)

     shapedrf(amplitudeTable, phaseTable, phase1, phase2, freq1, freq2, tableSize, tableStep)
...
short ShapedRFDualChannel(DLLParameters* par, char *args)
{
   ...
   if((nrArgs = ArgScan(par->itfc,args,8,"atable, stable, phase1, phase2, f1, f2, table_size, table_step","eeeeeeee","qqqqqqqq",&atable,&ptable,&phase1,&phase2,&freq1,&freq2,&size,&duration)) < 0)
      return(nrArgs);
   if(!parList)
   {
      ErrorMessage("Pulse sequence not initialised");
...
// Load step length
   ...
   fprintf(fp,"\n        lsl     #1,a                     ; Multiply step delay by two as we use a 10 ns delay step");
   fprintf(fp,"\n        sub     #405,a                   ; Adjust the step length allowing for other delays");
   fprintf(fp,"\n        move    a1,a0");
   fprintf(fp,"\n        rep     a0                       ; Wait until the correct step length is reached");
   fprintf(fp,"\n        nop");

// Calculate delay for subsequent steps
   fprintf(fp,"\n        add     #150,a                   ; Modify delay value to correct subsequent steps");
   fprintf(fp,"\n        move    a1,a0");
...
short SwitchOnBothTxChannels(DLLParameters* par, char *args)
{
  ...
  if((nrArgs = ArgScan(par->itfc,args,8,"ch1, amp1, phase1, freq1, ch2, amp2, phase2, freq2","eeeeeeee","qqqqqqqq",&ch1,&amp1,&phase1,&freq1,&ch2,&amp2,&phase2,&freq2)) < 0)
   ...
   if(amp1[0] == 'a' || amp1[0] == 't')
      InsertUniqueStringIntoList(amp1.Str(),&parList,szList);
   if(freq1[0] == 'f' || freq1[0] == 't')
      InsertUniqueStringIntoList(freq1.Str(),&parList,szList);
   if(phase1[0] == 'p' || phase1[0] == 't' )
      InsertUniqueStringIntoList(phase1.Str(),&parList,szList);
   if(amp2[0] == 'a' || amp2[0] == 't')
      InsertUniqueStringIntoList(amp2.Str(),&parList,szList);
   if(freq2[0] == 'f' || freq2[0] == 't')
      InsertUniqueStringIntoList(freq2.Str(),&parList,szList);
...
short SwitchOffTx(DLLParameters* par, char *args)
{
   ...
   if(nrArgs == 0 || mode == "w") // Switch off both channels
   {
      ...
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0      ; Zero phase");
      fprintf(fp,"\n        move    x:TXF10,a1");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0      ; Set correct frequency for channel 1");
      fprintf(fp,"\n        move    x:TXF11,a1");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
      ...
      fprintf(fp,"\n        move    a1,x:FPGA_DDS2_Pro0      ; Zero phase");
      fprintf(fp,"\n        move    x:TXF20,a1");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS2_Pro0      ; Set correct frequency for channel 2");
      fprintf(fp,"\n        move    x:TXF21,a1");
      fprintf(fp,"\n        move    a1,x:FPGA_DDS2_Pro0");
...
/**********************************************************************
     ...
     Takes 150 ns to set up level. The passed TTL level is shifted up 
     by 8 bits to prevent any disruption to the RF.
**********************************************************************/
...
short ClearData(DLLParameters* par, char *args)
{
   ...
   if((nrArgs = ArgScan(par->itfc,args,1,"number","ee","qq",&nrPnts, &nrSums)) < 0)
      return(nrArgs);
   ...
   if(nrPnts[0] == 'n')
      InsertUniqueStringIntoList(nrPnts.Str(),&parList,szList);
   if (nrSums[0] == 'n')
      InsertUniqueStringIntoList(nrSums.Str(), &parList, szList);
...
// Get mode
   ...
   {
      if((nrArgs = ArgScan(par->itfc,args,3,"mode, nr points, shift factor, [duration]","eeee","qqqq",&mode,&nrPnts,&shift,&duration)) < 0)
         return(nrArgs);

   // Add new variables to the list
      InsertUniqueStringIntoList(nrPnts.Str(),&parList,szList);
      InsertUniqueStringIntoList(shift.Str(),&parList,szList);
      if(nrArgs >= 3)
         InsertUniqueStringIntoList(duration.Str(),&parList,szList);
   }
   else if (mode == "sum" || mode == "stack")
   {
      if ((nrArgs = ArgScan(par->itfc, args, 3, "mode, nr points, duration, [shift factor, saveAdrs]", "ccccc", "ttttt", &mode, &nrPnts, &duration, &shift, &adrs)) < 0)
         return(nrArgs);
...
// Overwrite mode
   ...
   else if(mode == "integrateandscale")
   {
      if(nrArgs == 2)
      {
         fprintf(fp,"\n;");
         fprintf(fp,"\n;***************************************************************************");
         fprintf(fp,"\n; Acquire (integrating and scaling without delay)");
         ...
         fprintf(fp,"\n        clr     a                       ; Clear real accumulator");
         fprintf(fp,"\n        clr     b                       ; Clear imaginary accumulator");

         if(SelectNumber(par->itfc,fp,nrPnts,"r7",2,65536))
            return(ERR);
         ...
         fprintf(fp,"\n        move    x:FPGA_SampleA,x0       ; Load data from channel A");
         fprintf(fp,"\n        nop");
         fprintf(fp,"\n        nop");
         fprintf(fp,"\n        add     x0,a                    ; Accumulate in a");

         fprintf(fp,"\n        move    x:FPGA_SampleB,x0       ; Load data from channel B");
         fprintf(fp,"\n        nop");
         fprintf(fp,"\n        nop");
         fprintf(fp,"\n        add     x0,b                    ; Accumulate in b");

         fprintf(fp,"\nLBL%ld  nop",label++);

         if(SelectNumber(par->itfc,fp,shift,"r7",2,65536))
            return(ERR);

         fprintf(fp,"\n        do      r7,LBL%ld                 ; Scale data down",label);
         fprintf(fp,"\n        asr     a");
         fprintf(fp,"\n        asr     b");
         fprintf(fp,"\nLBL%ld  nop",label++);

         fprintf(fp,"\n        move    a1,y:(r5)+              ; Write real part to memory");
         fprintf(fp,"\n        move    b1,y:(r5)+              ; Write imaginary part to memory");
      ...
      else
      {
         fprintf(fp,"\n;");
         fprintf(fp,"\n;***************************************************************************");
         fprintf(fp,"\n; Acquire (integrating with scaling and delay)");
...
short SelectNumber(Interface *itfc, FILE* fp, CText num, char *reg, long min, long max)
{
   ...
   else if (num[0] == 't')
   {
      fprintf(fp, "\n        clr a");
      fprintf(fp, "\n        clr b");
      fprintf(fp, "\n        move    x:TABLE%s,a0", num.Str() + 1);
      fprintf(fp, "\n        dec a"); // a0 points to table index
      fprintf(fp, "\n        move    a0,r5"); // Read current table index
      fprintf(fp, "\n        move    y:(r5),a0");

      fprintf(fp, "\n        move    x:TABLE%s,b0", num.Str() + 1);
...
short SelectPhaseRampedPulse(Interface* itfc, FILE* fp, CText phase, CText channel)
{
   ...
   // Send to appropriate FPGA channel
   if (channel == "1")
      fprintf(fp, "\n        move    a1,x:FPGA_DDS1_POW");
   else
      fprintf(fp, "\n        move    a1,x:FPGA_DDS2_POW");
...
short SelectAmplitude(Interface *itfc ,FILE* fp, CText amp, CText channel)
{
   ...
   else if(amp[0] == 't') // Amplitude is a table - note that registers a, b and r5 as well as reg are modified
   {
      fprintf(fp, "\n        clr a");
      fprintf(fp, "\n        clr b");
...
short SelectAmplitudeRamped(Interface *itfc ,FILE* fp, CText amp, CText steps, CText channel, long &nrSteps)
{
   long Amplitude;
   long stepsExponent = 3;

// Get the main pulse amplitude
   if(amp[0] == 'a')
   {
      fprintf(fp,"\n        move    x:TXA%s,a1",amp.Str()+1);
   }
...
// Divide the amplitude by 2^steps to find the step size - return in b1
   if(sscanf(steps.Str(),"%ld",&stepsExponent) == 1)
   {
      if(stepsExponent < 1 || stepsExponent > 8)
      {
         Error(itfc,"Invalid step exponent should be 1 -> 8 ");
         fclose(fp);
         return(ERR);
      }
      fprintf(fp,"\n        lsr    #%ld,a1",stepsExponent);
      nrSteps = 1 << stepsExponent;
   }

// Load a1 with the initial amplitude
   fprintf(fp,"\n        clr    b");
   fprintf(fp,"\n        move    a1,b1");
...
// Select the shim channel
   if(SelectShim16(par->itfc,fp,channel)) return(ERR);

// Select the shim amplitude
   if(SelectNumber(par->itfc, fp, amplitude, "a1", -32768, 32767)) return(ERR);
 ...
 /*  if(amplitude[0] != 'n')
   {
      if(sscanf(amplitude.Str(),"%ld",&Amplitude) == 1)
      {
         ...
         {
            Error(par->itfc,"invalid shim amplitude '%s' [-32767 ... 32768]",amplitude);
            fclose(fp);
            return(ERR);
         }
      }
...
// Version which fixes zero crossing and allows tables
//short SwitchOnShim16(DLLParameters* par, char *args)
//{
//   short nrArgs;
//   CText channel,amplitude;
...
//   }
//
// // Correct for zero crossing error
//   fprintf(fp,"\n; Correct for zero crossing error");
// fprintf(fp,"\n        move    #0,a0");
//
//   fprintf(fp,"\n        cmp     #000000,a"); // Check for zero - do nothing in this case
// fprintf(fp,"\n        jeq     LBL%ld",label+1);
//   fprintf(fp,"\n        cmp     #800000,a"); // Check for negative number
//
// fprintf(fp,"\n        jge     LBL%ld",label); // Its negative
//
//   fprintf(fp,"\n        add     #14,a");  // positive so add offset
//   fprintf(fp,"\n        jmp     LBL%ld",label+1);      
//
// fprintf(fp,"\nLBL%ld    sub     #14,a",label); // Negative so subtract offset
//
// fprintf(fp,"\nLBL%ld    nop",label+1);
// label += 2;
//
//// Send value to shim board
//   fprintf(fp,"\n; Send value to shim board");
//   fprintf(fp,"\n        move    #$00FFFF,x1"); 
//   fprintf(fp,"\n        and      x1,a1");  // Mask out data
//   fprintf(fp,"\n        move     y:(TMP+10),b0");
...
// Choose the gradient channel
   ...
   if(grad[0] == 'n') // Channel is a variable
   {
      fprintf(fp,"\n        move    x:NR%s,a1",grad.Str()+1);
   }
   else if(sscanf(grad.Str(),"%ld",&Channel) == 1) // Channel is a constant number
   {
...
//
// This sends a message to the lock board to stop the lock and generate a saturation pulse. 
//    Then it waits for a T1 signal on the backplane to show this is finished. The
//  rest of the sequence then proceeds
//

short StopLockAndSaturate(DLLParameters* par, char *args)
{
   FILE *fp = fopen("midCode.asm","a");
   if(!fp)
   {
   ...
   fprintf(fp,"\n\n;***************************************************************************");
   fprintf(fp,"\n; Suspend lock but tell uBlaze to saturate first");
   fprintf(fp,"\n        clr     a"); 
   fprintf(fp,"\n        move    a1,x:$28480");
   fprintf(fp,"\n        move    #2,a1");
   fprintf(fp,"\n        move    a1,x:$28481");
   fprintf(fp,"\n; Wait for T1 pulse after lock sat pulse");
   fprintf(fp,"\n        movep   #0,x:A_TLR2             ; Set up ADC timer");
   fprintf(fp,"\n        movep   #0,x:A_TCSR2            ; Disable timer");
   fprintf(fp,"\n        movep   #$361,x:A_TCSR2         ; Set up timer1 (T1) for input capture");
   fprintf(fp,"\n        jclr    #21,x:A_TCSR2,*         ; Wait for timer1 flag");
   fprintf(fp,"\n        movep   #$200361,x:A_TCSR2      ; Clear timer1 flag");
   fprintf(fp,"\n        movep   #$200A00,x:A_TCSR2      ; Turn off timer");
...
/**********************************************************************
     Generate a shaped shim pulse

     shapedshim(grad, amplitudeTable, tableSize, tableStep)

     shim ............. which shim to control (1 .. 15)
     amplitudeTable ... table of amplitudes (-2^15 -> 2^15-1)
     tableSize ........ number of value in the table
     tableStep ........ duration of each table value (in us)

     shapedshim(n?, t?, n?/constant, d?/constant)

     Two other options exist for controlling more than one shim
     simultaneously:

     shapedshim(shim1, table1, shim2, table2, tableSize, tableStep)

     shapedshim(shim1, table1, shim2, table2, shim3, table3, tableSize, tableStep)

**********************************************************************/

short ShapedShim(DLLParameters* par, char* args)
{
   short nrArgs;
   long Size, Duration;
   float fDuration;
   ...
   if (n == 4)
   {
      CText shim, atable, size, duration;

      if ((nrArgs = ArgScan(par->itfc, args, 4, "shim, atable, table_size, table_step", "eeee", "qqqq", &shim, &atable, &size, &duration)) < 0)
         return(nrArgs);
      ...
      // Set up parameter list
      if (shim[0] == 'n')
         InsertUniqueStringIntoList(shim.Str(), &parList, szList);
      InsertUniqueStringIntoList(atable.Str(), &parList, szList);
      if (size[0] == 'n')
         InsertUniqueStringIntoList(size.Str(), &parList, szList);
      if (duration[0] == 'd')
     ...
     // Set the serial port
      ...
      if (SelectShim16(par->itfc, fp, shim))
         return(ERR);
      ...
      else if(sscanf(size.Str(), "%ld", &Size) == 1)
      {
         if(Size < 1)
         {
            Error(par->itfc, "invalid table size '%s' [>= 1]", shim);
            fclose(fp);
            return(ERR);
         }
         fprintf(fp, "\n        move    #%ld,r2", Size);
      ...
      fprintf(fp, "\n        and      #$00FFFF,a             ; Remove upper byte ");
      fprintf(fp, "\n        or      x1,a                    ; Add channel info");
      fprintf(fp, "\n        move    a1,x:A_TX10             ; Set up gradient level");
   ...
   // Two shims controlled ************************************

   else if (n == 6)
   {
      CText shim1, atable1, shim2, atable2, size, duration;

      if ((nrArgs = ArgScan(par->itfc, args, 6, "adrs1, atable1, adrs2, atable2, table_size, table_step", "eeeeee", "qqqqqq", &shim1, &atable1, &shim2, &atable2, &size, &duration)) < 0)
         return(nrArgs);
      ...
      // Set up parameter list
      if (shim1[0] == 'n')
         InsertUniqueStringIntoList(shim1.Str(), &parList, szList);
      InsertUniqueStringIntoList(atable1.Str(), &parList, szList);
      if (shim2[0] == 'n')
         InsertUniqueStringIntoList(shim2.Str(), &parList, szList);
      ...
      // Extract the table addresses for the two shims
      ...
      else if (sscanf(size.Str(), "%ld", &Size) == 1)
      {
         if (Size < 1)
         {
            Error(par->itfc, "invalid table size '%s' [>= 1]", shim2);
            fclose(fp);
            return(ERR);
         }
         fprintf(fp, "\n        move    #%ld,r2", Size);
      ...
      // Loop over the tables
      fprintf(fp, "\n        do      r2,LBL%ld               ; Step the amplitude r2 times", label + 8);

      if (SelectShim16(par->itfc, fp, shim1)) return(ERR);
      ...
      fprintf(fp, "\n        and     #$00FFFF,a              ; Remove upper byte ");
      fprintf(fp, "\n        or      x1,a                    ; Add channel info");
      fprintf(fp, "\n        movep   a1,x:A_TX10             ; Set up shim level");

      fprintf(fp, "\n        rep     #185                    ; Wait 2us");
      fprintf(fp, "\n        nop");

      if (SelectShim16(par->itfc, fp, shim2)) return(ERR);
   ...
   // Three shims controlled ************************************

   else if (n == 8)
   {
      CText shim1, atable1, shim2, atable2, shim3, atable3, size, duration;

      if ((nrArgs = ArgScan(par->itfc, args, 8, "adrs1, atable1, adrs2, atable2, adrs3, atable3, table_size, table_step", "eeeeeeee", "qqqqqqqq", &shim1, &atable1, &shim2, &atable2, &shim3, &atable3, &size, &duration)) < 0)
         return(nrArgs);
      ...
      InsertUniqueStringIntoList(atable2.Str(), &parList, szList);
      if (shim3[0] == 'n')
         InsertUniqueStringIntoList(shim3.Str(), &parList, szList);
      ...
      fprintf(fp, "\n;");
      fprintf(fp, "\n;***************************************************************************");
      fprintf(fp, "\n; Make  3 shaped shim pulses");

      fprintf(fp, "\n        movep   #$2C,x:A_PCRD          ; Turn on SSI 1 on Port D");

      // Extract the table addresses for the three shims
...
// Extract the table sizes
      ...
      if (SelectShim16(par->itfc, fp, shim3)) return(ERR);
