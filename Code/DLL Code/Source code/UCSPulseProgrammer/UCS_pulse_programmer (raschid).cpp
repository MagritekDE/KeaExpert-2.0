...
1.33 -------------------------------
   1. Test version with rampedpulse2 and rampedpulse2setup

   Last modified 1 September 2021 CDE

	1.33.1 Raschids first modifcation
...
short RampedRFPulse2(DLLParameters*,char*);
short RampedRFPulse2SetUp(DLLParameters*,char*);
...
EXPORT short  AddCommands(char *command, char *parameters, DLLParameters *dpar)
{
 ...
 //  else if(!strcmp(command,"execwait"))          r = ExecuteAndWait(dpar,parameters);
   ...
   else if(!strcmp(command,"rampedpulse2"))      r = RampedRFPulse2(dpar,parameters);    
   else if(!strcmp(command,"rampedpulse2setup")) r = RampedRFPulse2SetUp(dpar,parameters);    
...
EXPORT void  ListCommands(void)
{
   TextMessage("\n\n   UCS (Proton/Carbon) Pulse Programmer DLL module (V%1.2f) for Standard Software\n\n",VERSION);
 ...
 //  TextMessage("   execwait ........ execute a program and wait for it to exit\n");
   ...
   TextMessage("   rampedpulse2 .... generate an RF pulse with fast ramped ends\n");
   TextMessage("   rampedpulse2setup setup code needed to generate rampedpulse2\n");   
...
EXPORT bool GetCommandSyntax(char* cmd, char* syntax)
{
 ...
 //  else if(!strcmp(cmd,"execwait"))          strcpy(syntax,"execwait(program,arguments)");
   ...
   else if(!strcmp(cmd,"rampedpulse2"))      strcpy(syntax,"rampedpulse2(channel (1/2), dummy_amplitude: a, phase:p, duration:d");
   else if(!strcmp(cmd,"rampedpulse2setup")) strcpy(syntax,"rampedpulse2setup(channel (1/2), amp:a, [frequency:f]");
...
/*********************************************************************************************************
 Set up a fast-ramped pulse. This needs to be the first command in the sequence
**********************************************************************************************************/

short RampedRFPulse2SetUp(DLLParameters* par,char *args)
{
   short nrArgs;
...
/*********************************************************************************************************
 Generate a fast-ramped pulse. (Note that the setup code must be present)
**********************************************************************************************************/
//
//short RampedRFPulse2(DLLParameters* par,char *args)
//{
//   short nrArgs;
...
//      fprintf(fp,"\n        jlt    ABORT                     ; Abort if not");                 
//
//   // Reset profile 0 in case a normal pulse has been run before
//      fprintf(fp,"\n; Set profile 0");
//      fprintf(fp,"\nLBL%ld  move    #$000000,a1",label++); 
//      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
//      fprintf(fp,"\n        move    #$002003,a1             ; step rate 32 (128ns), endAddr 36"); 
//      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
//      fprintf(fp,"\n        move    #$00c000,a1             ; start addr 0"); 
...
//      fprintf(fp,"\n        clr     a"); 
//      fprintf(fp,"\n        move    x:DELAY%s,a1",duration.Str()+1);
//      fprintf(fp,"\n        sub     #202,a                  ; subtract 2us from pulse length for the added ramps"); 
//      fprintf(fp,"\n        lsr     #8,a");
//      fprintf(fp,"\n        and     #$FF,a"); 
//      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro1");
//      fprintf(fp,"\n        move    x:DELAY%s,a1",duration.Str()+1);
...
//      fprintf(fp,"\n        move    y:TTL,a1                ; Load the current TTL level");
//      if(channel == "1") 
//         fprintf(fp,"\n        or      #$004000,a              ; Switch on RF amplifier gate (Channel 1)");
//
//      fprintf(fp,"\n        move    a1,y:TTL                ; Save new TTL word");
//      fprintf(fp,"\n        move    a1,x:FPGA_TTL           ; Update TTL RF remains the same");
//   // Wait for time pgo
...
//      fprintf(fp,"\n        jclr    #21,x:A_TCSR2,*         ; Wait for parameters to update");
//
//   // Trigger RAM pulse (~1us processing latency)
//      fprintf(fp,"\n        move    #$00c005,a1                 ; CFR1 32-16  RAM enabled, RAM destination = AMP, Internal Profile Control Mode (Pro0-Pro2 then halt), Cosine"); 
//      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_CFR1");
//      fprintf(fp,"\n        move    #$000000,a1                 ; CFR1 15-0");      
//      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_CFR1");
//   // Start pulse / unblank
//      fprintf(fp,"\n        move    x:DELAY%s,a1",duration.Str()+1);
//      fprintf(fp,"\n        add     #50,a               ; extend RF on duration to account for added ramps");
//      fprintf(fp,"\n        movep   a1,x:A_TCPR2");
//      fprintf(fp,"\n        nop");
//      fprintf(fp,"\n        move    y:TTL,a1                ; Load the current TTL word");
//      fprintf(fp,"\n        or      #$000008,a              ; Channel 1 RF on");
...
//      fprintf(fp,"\n        move    a1,x:FPGA_TTL              ; Update TTL & RF");
//
//      fprintf(fp,"\n        move    #$004001,a1                ; CFR1 32-16   RAM disabled, RAM destination = AMP");    
//      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_CFR1");
//      fprintf(fp,"\n        move    #$000000,a1                 ; CFR1 15-0");      
//      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_CFR1");
//      fprintf(fp,"\n        move    #2,r7"); 
...
//      fprintf(fp,"\n        move    y:TTL,a1                ; Load the current TTL level");
//      if(channel == "2") 
//         fprintf(fp,"\n        or      #$000100,a              ; Switch on RF amplifier gate (Channel 2)");
//
//      fprintf(fp,"\n        move    a1,y:TTL                ; Save new TTL word");
//      fprintf(fp,"\n        move    a1,x:FPGA_TTL           ; Update TTL RF remains the same");
//   // Wait for time pgo
...
short RampedRFPulse2(DLLParameters* par,char *args)
{
   ...
   // Calculate and set step rate values for AD9910 channel 1 profile 1 based on the pulse duration
      ...
      fprintf(fp,"\n        clr     a"); 
      fprintf(fp,"\n        move    x:DELAY%s,a1",duration.Str()+1);
      fprintf(fp,"\n        sub     #202,a                  ; subtract 4us from pulse length for the added ramps"); 
      fprintf(fp,"\n        lsr     #8,a");
      fprintf(fp,"\n        and     #$FF,a"); 
      fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro1");
      fprintf(fp,"\n        move    x:DELAY%s,a1",duration.Str()+1);
