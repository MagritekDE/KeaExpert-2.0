/*************************************************************
     ...
     Special version with controlled timing 
...
#define VERSION 1.2 // 17-8-17

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Version history
// 
// 1.0 - modified version of the normal pulse programmer without using the 20 ns time rather
//       nops are used to set the timing.  (8-1-14)
// 1.1 - addition of gradon, gradoff, gradramp and wait commands modified to give accurate timing. (6-5-16)
// 1.2 - addition of 'sum' mode to the acquire copmmand (17-8-17)
//
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
short InitPhase(DLLParameters*,char*);
...
EXPORT short  AddCommands(char *command, char *parameters, DLLParameters *dpar)
{
   ...
   else if(!strcmp(command,"pulseinit"))        r = MakeAnRFPulseInit(dpar,parameters);    
   else if(!strcmp(command,"initrf"))    r = InitPhase(dpar,parameters);    
   else if(!strcmp(command,"test"))         r = TimeTest(dpar,parameters);      
...
EXPORT void  ListCommands(void)
{
   TextMessage("\n\n   Kea-2 Hi-Res (10ns) Pulse Programmer DLL module (V%1.2f)\n\n",VERSION);
   ...
   TextMessage("   gradramp .... change one or more gradients using a linear ramp\n");
   TextMessage("   initpp ...... initialise pulse program\n");
   TextMessage("   loop ........ start a loop\n");
   TextMessage("   nop ......... add a 10 ns delay\n");
   TextMessage("   initrf....... initialise rf phase to zero\n");
   TextMessage("   ppversion ... returns the version number of this DLL\n");
   TextMessage("   pulse ....... generate an RF pulse\n");
   TextMessage("   pulseinit ....... generate an RF pulse\n");
...
EXPORT bool GetCommandSyntax(char* cmd, char* syntax)
{
   ...
   if(!strcmp(cmd,"acquire"))            strcpy(syntax,"acquire(mode, number points:n, [duration:d, [data address:m]])");
   ...
   else if(!strcmp(cmd,"initrf"))        strcpy(syntax,"initrf()");
	...
	else if(!strcmp(cmd,"pulseinit"))         strcpy(syntax,"pulseinit(mode, amp:a, phase:p, duration:d [,freq:f] OR pulse(ch1 ,a1, p1, f1, ch2 ,a2, p2, f2, d])");
...
/**********************************************************************
     Initialise the phase to 0 

     initphase() DOESN'T WORK

**********************************************************************/

short InitPhase(DLLParameters* par, char *args)
{
   if(!parList)
   {
   ...
   fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0"); // Set phase
	fprintf(fp,"\n        move    x:TXF00,a1");
	fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0"); // Set frequency
	fprintf(fp,"\n        move    x:TXF01,a1");
	fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");

   // THIS DOES NOT WORK BECAUSE THERE IS A PHASE MEMORY BEFORE THIS COMMAND - IT DOES NOT RESET THE PHASE TO ZERO
   // RESETTING THE PHASE/FREQUENCY IS A NONTRIVIAL CHANGE.
...
short MakeAnRFPulseInit(DLLParameters* par, char *args)
{
  short nrArgs;
   ...
   // Wait for pulse to end
		...
		if(SelectDuration(par->itfc ,fp, "b1", duration) == ERR) return(ERR);
      fprintf(fp,"\n        sub     #27,b"); // Tweek it
      fprintf(fp,"\n        rep     b");
      fprintf(fp,"\n        nop");
	...
	// End RF pulse
      ...
      fprintf(fp,"\n        move    a1,y:TTL                 ; Update TTL word");
		...
		if(channel == "2")
		{
			fprintf(fp,"\n        move    b1,x:FPGA_DDS2_Pro0      ;Zero amplitude");
		}
		else
		{
			fprintf(fp,"\n        move    b1,x:FPGA_DDS1_Pro0      ;Zero amplitude");
		}
	   fprintf(fp,"\n        move    a1,x:FPGA_TTL           ; Update TTL & RF");
...
short MakeADelay(DLLParameters* par, char *args)
{
   ...
   {
      if(fDuration < 0.10 || fDuration > 327670)
      {
         Error(par->itfc,"invalid delay '%f' [0.25 -> 327670]",fDuration);
         fclose(fp);
         return(ERR);
      }
      Duration = (long)(fDuration * 100 - 1 + 0.5);
      fprintf(fp,"\n        clr    a");
      fprintf(fp,"\n        move    #%ld,a1",Duration);
      fprintf(fp,"\n        sub     #8,a"); // Tweek it
      fprintf(fp,"\n        rep     a");
...
short MakeALongDelay(DLLParameters* par, char *args)
{
   ...
   {
      if(fWait < 0.2 || fWait > 167e6)
      {
         Error(par->itfc,"invalid delay '%ld' [0.2 us...167 s]",wait);
         fclose(fp);
         return(ERR);
      }
      ...
      if(fWait/16777216 < 1) // Can be represented in < 2^24 1us steps?
      {
         ...
         if(in > 1)
         {
            ...
            delay1 = 103 + (long)(fr*100+0.5) - 22; // Remaining time as 10 ns steps was 19
            delay2 = 95; // Fiddle factor
         }
         else if(fWait > 0.2)
         {
            nrSteps = 0;
            delay1 = (long)(fWait*100+0.5) - 22; // From 20 to 22
            delay2 = 0;
         }
      }
      else if(fWait/16777216 < 10) // Can be represented in < 2^24 10 us steps?
      {
...
/**********************************************************************
     ...
     Takes 130 ns to set up level. The TTL level is combined with the
...
short TimeTest(DLLParameters* par, char *args)
{
   short nrArgs;
   CText byte;
   long Byte;
...
/**********************************************************************
     ...
     Takes 140 ns to set up level. The TTL level is combined with the
     lower 16 bits to prevent any disruption to the RF.
**********************************************************************/
...
short AcquireData(DLLParameters* par, char *args)
{
   ...
   if((nrArgs = ArgScan(par->itfc,args,2,"mode, number points","ee","qq",&mode,&nrPnts)) < 0)
      return(nrArgs);
...
// Overwrite mode
   if(mode == "overwrite") // Each data point is stored in a new location, subsequent calls overwrite this
   {
      ...
      fprintf(fp,"\n        move    #10,a1"); // Make the duration same as integrate
      fprintf(fp,"\n        rep     a");
      fprintf(fp,"\n        nop");
      label += 1;
		fclose(fp);
	...
	else if(mode == "append") // Each data point is stored in a new location, subsequent calls append to this
	{
      ...
      fprintf(fp,"\n        move    #09,a1"); // Make the duration same as integrate
      fprintf(fp,"\n        rep     a");
      fprintf(fp,"\n        nop");
		fclose(fp);
