...
short ShapedGradient15Pulse(DLLParameters* par, char *args);
...
short SwitchOnShim(DLLParameters*,char*);
short SwitchOnShim16(DLLParameters*,char*);
short SwitchOnGrad15(DLLParameters*,char*);
...
short RampedGradient15(DLLParameters* par, char *args);
...
EXPORT short  AddCommands(char *command, char *parameters, DLLParameters *dpar)
{
   ...
   else if(!strcmp(command,"grad15"))       r = SwitchOnGrad15(dpar,parameters);   
   ...
   else if(!strcmp(command,"gradramp15"))   r = RampedGradient15(dpar,parameters);   
   else if(!strcmp(command,"gradramp16"))   r = RampedGradient16(dpar,parameters);   
  ...
  // else if(!strcmp(command,"setlfrxoffset2"))    r = SetLowFreqOffset2(dpar,parameters);   
   ...
   else if(!strcmp(command,"shapedgrad15")) r = ShapedGradient15Pulse(dpar,parameters);   
   else if(!strcmp(command,"shapedrf"))     r = ShapedRF(dpar,parameters);  
   else if(!strcmp(command,"shim"))         r = SwitchOnShim(dpar,parameters);   
 ...
 //  else if(!strcmp(command,"updatetx"))     r = UpdateTx(dpar,parameters);
...
EXPORT void  ListCommands(void)
{
   ...
   TextMessage("   grad15 ...... set a gradient (15 channel board)\n");
   ...
   TextMessage("   gradramp15 .. change a gradient using a linear ramp (15 channel)\n");
   TextMessage("   gradramp16 .. change a gradient using a linear ramp (16 channel)\n");
   ...
   TextMessage("   shapedgrad15  make an amplitude modulated gradient pulse (15 channel controller)\n");
   TextMessage("   shim ........ set an 8 channel shim current\n");
 ...
 //  TextMessage("   updatetx .... update the transmit parameters\n");
...
EXPORT bool GetCommandSyntax(char* cmd, char* syntax)
{
   ...
   else if(!strcmp(cmd,"grad15"))        strcpy(syntax,"grad15(address:n,level:n/t)");
   ...
   else if(!strcmp(cmd,"shapedgrad15"))  strcpy(syntax,"shapedgrad15(address:n, atable:t, table_size:n, table_step_duration:d)");
   else if(!strcmp(cmd,"shim"))          strcpy(syntax,"shim(channel:n, amplitude:n)");
 ...
 //  else if(!strcmp(cmd,"updatetx"))      strcpy(syntax,"updatetx(mode, amp:a, freq:f, phase:p)");
...
/**********************************************************************
     Generate a shaped gradient pulse - 15 channel (18/10/11)

     shapedgrad(grad, amplitudeTable, tableSize, tableStep)

     grad ............. which gradient to control (1 .. 15)
     amplitudeTable ... table of amplitudes (-2^15 -> 2^15-1)
     tableSize ........ number of value in the table
     tableStep ........ duration of each table value (in us)

     shapedgrad15(n?, t?, n?/constant, d?/constant)

     Two other options exist for controlling more than one gradient
     simultaneously:

     shapedgrad15(grad1, table1, grad2, table2, tableSize, tableStep)

     shapedgrad15(grad1, table1, grad2, table2, grad3, table3, tableSize, tableStep)
...
// Set up parameter list
      ...
      if(SelectGradient15(par->itfc,fp,grad))
         return(ERR);
...
// Loop over the tables
      fprintf(fp,"\n        do      r2,LBL%ld               ; Step the amplitude r2 times",label+2);

      if(SelectGradient15(par->itfc,fp,grad1)) return(ERR);
      fprintf(fp,"\n        clr     a");
      fprintf(fp,"\n        move    y:(r4)+,a1              ; Load next amplitude"); 
      fprintf(fp,"\n        add     #32768,a");
      fprintf(fp,"\n        lsl     #8,a");    
      ...
      if(SelectGradient15(par->itfc,fp,grad2)) return(ERR);
      fprintf(fp,"\n        clr     a");
      fprintf(fp,"\n        move    y:(r5)+,a1              ; Load next amplitude");
      fprintf(fp,"\n        add     #32768,a");
      fprintf(fp,"\n        lsl     #8,a");    
...
// Loop over the tables
      ...
      if(SelectGradient15(par->itfc,fp,grad3)) return(ERR);
      fprintf(fp,"\n        movep   a1,x:A_PDRE");
      fprintf(fp,"\n        move    y:(r1)+,a1              ; Load next amplitude"); 
      fprintf(fp,"\n        add     #32768,a");
      fprintf(fp,"\n        lsl     #8,a");    
...
short SwitchOnOneTxChannel(DLLParameters* par, char *args, short nrArgs)
{
   ...
   {
      if(channel == "i" || channel == "e" || channel == "1" || channel == "w1")
      {
		   fprintf(fp,"\n        move    x:TXF00,a1");
...
// Get the channel number
   ...
   fprintf(fp,"\n        movep   #$180803,x:A_CRA1       ; /3 clk, 24 bit word transferred"); 
...
/**********************************************************************
     ...
     Note this is designed to work with the 15-channel shim controller

**********************************************************************/

short SwitchOnGrad15(DLLParameters* par, char *args)
{
   short nrArgs;
   CText channel,amplitude;
   long Channel, Amplitude;
...
// Add to parameter list if not a constant
   ...
   fprintf(fp,"\n;");
   fprintf(fp,"\n;***************************************************************************");
   fprintf(fp,"\n; Set Gradient level (15 channel)");


// Choose the gradient level
   fprintf(fp,"\n        movep   #$2C,x:A_PCRD          ; Turn on SSI 1 on Port D");

   if(SelectGradient15(par->itfc,fp,channel)) return(ERR);
...
// Overwrite mode
   if(mode == "overwrite") // Each data point is stored in a new location, subsequent calls overwrite this
   {
		...
		//	fprintf(fp,"\n        sub     #5,a"); // Tweek it
         ...
         if(nrArgs == 4)
             ...
             fprintf(fp,"\n        move    y:DATA_ADRS,r5           ; Specify the current data address",adrs.Str()+1);
		   if(SelectNumber(par->itfc,fp,nrPnts,"r7",2,65536)) return(ERR);
         fprintf(fp,"\n        do      r7,LBL%ld                ; Collect n samples",label);
         fprintf(fp,"\n        jclr    #21,x:A_TCSR1,*         ; Wait for timer1 flag");
			fprintf(fp,"\n        movep   #$200361,x:A_TCSR1      ; Clear timer1 flag");
   ...
   else if(mode == "integrate") // All points are added together and stored in the next memory location
   {
		if(nrArgs == 2) // No delay
		{
         ...
         fprintf(fp,"\n        move    r5,a1");
         fprintf(fp,"\n        add     #2,a");
         fprintf(fp,"\n        move    a1,y:DATA_ADRS           ; Save data address",adrs.Str()+1);
...
// Overwrite mode
   if(mode == "overwrite") // Each data point is stored in a new location, subsequent calls overwrite this
   {
	   fprintf(fp,"\n\n;");
	   fprintf(fp,"\n;***************************************************************************");
	   fprintf(fp,"\n; Stop acquiring data (overwrite without delay)");
...
/*******************************************************************************
   ...
   This version is for the 15 channel gradient controller
...
// Select the gradient
   if(SelectGradient15(par->itfc,fp,grad)) return(ERR);
...
short SelectFrequency(Interface *itfc, FILE*fp, CText freq, CText channel)
{
   ...
   {
	   if(channel == "1" || channel == "i" || channel == "e" ||
         channel == "n" || channel == "w1" || channel == "wi" || channel == "we")
	   {
		   fprintf(fp,"\n        move    x:FX%s_0,a1",freq.Str()+1);
		   fprintf(fp,"\n        move    a1,x:FPGA_DDS1_Pro0");
		   fprintf(fp,"\n        move    x:FX%s_1,a1",freq.Str()+1);
