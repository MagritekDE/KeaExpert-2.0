...
* 1.10 Fixed the error code returned by setupUSB. (Was returning an error = 0)
*
* Last modified 26/01/24 CDE
*
*************************************************************************/ 
...
WINUSB_INTERFACE_HANDLE setupUSB(HANDLE &hDeviceHandle, PIPE_ID &PipeID, PIPE_ID &intPipeID, long usbPort, int *err=0);
...
// Open device *******
   hWinUSBHandle = setupUSB(hDeviceHandle,pipeID,intPipeID,usbPort, &err);
...
// poll for an abort (either the abort button pressed or escape).
   ...
   if (threadflag[usbPort] == 4)
      par->retVar[1].MakeAndSetString("usb_error");
...
BOOL GetDeviceHandle (GUID guidDeviceInterface, PHANDLE hDeviceHandle, long usbPort)
{
 ...
 // Check to see if device is present
    if (index == 0)
    {
      // TextMessage("\n   USB error: No DSP devices connected\n");
       goto done;
    }

    if(interfaceNumber + 1 > index)
    {
      //  TextMessage("\n   USB error: DSP device '%ld' is not connected\n",interfaceNumber);
        goto done;
    }
...
BOOL GetWinUSBHandle(HANDLE hDeviceHandle, PWINUSB_INTERFACE_HANDLE phWinUSBHandle)
{
    if (hDeviceHandle == INVALID_HANDLE_VALUE)
    {
        ErrorMessage("Invalid USB device handle. Error %d.", GetLastError());
        return FALSE;
    }
...
BOOL GetUSBDeviceSpeed(WINUSB_INTERFACE_HANDLE hWinUSBHandle, UCHAR* pDeviceSpeed)
{
    ...
    {
        TextMessage("Invalid USB handle or error getting device speed: %d.\n", GetLastError());
        return FALSE;
    }
...
BOOL QueryDeviceEndpoints (WINUSB_INTERFACE_HANDLE hWinUSBHandle, PIPE_ID* pipeid, PIPE_ID* intpipeid)
{
    if (hWinUSBHandle==INVALID_HANDLE_VALUE)
    {
        TextMessage("Invalid USB handle. Error: %d.\n", GetLastError());
        return FALSE;
    }
 ...
 //                   TextMessage("Endpoint index: %d Pipe type: Isochronous Pipe ID: %d.\n", index, Pipe.PipeType, Pipe.PipeId);
   ...
   /*                     TextMessage("Endpoint index: %d Pipe type: Bulk Pipe ID: %c.\n", index, Pipe.PipeType, Pipe.PipeId);
      ...
      //                  TextMessage("Endpoint index: %d Pipe type: Interrupt Pipe ID: %d.\n", index, Pipe.PipeType, Pipe.PipeId);
       ...
       TextMessage("Error reading USB endpoints. Error: %d.\n", GetLastError());

    return bResult;
}

WINUSB_INTERFACE_HANDLE setupUSB(HANDLE &hDeviceHandle, PIPE_ID &PipeID, PIPE_ID &intPipeID, long usbPort, int *err)
{
