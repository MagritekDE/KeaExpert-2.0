...
short EvaluateRealMatrix(Interface *itfc, char *operand, short dataType, Variable *result)
{
   ...
   char  row[MAX_STR];       // The row descriptor in text form
   char  element[MAX_STR];   // A particular element extract from row string
...
// Extract the first row in text form from 'operand'
	strcpy(row,cRows.Extract(1));

// Figure out row type and count the number of columns in the first row
   if(CheckMatrixRow(itfc, row, cRow, nrMatrixCols, start, step, end, mode) == ERR)
       return(ERR);
...
// Set the elements of each row in the newly formed matrix
   for(i = 0; i < nrMatrixRows; i++)
   {
      ...
      else // [a,b,c,d, ...]
      {
	      for(j = 0; j < nrMatrixCols; j++)
	      { 
	         strcpy(element,cRow.Extract(j+1));
		      if(Evaluate(itfc,RESPECT_ALIAS,element,result) != FLOAT32)
		      {
		         FreeMatrix2D(mat);
		         ErrorMessage("array index '%s' is not a number",element);
		         return(ERR);
		      } 
...
// Set the elements of each row in the newly formed matrix
   for(i = 0; i < nrMatrixRows; i++)
   {
      ...
      else // [a,b,c,d, ...]
      {
	      ...
	      { 
	         strcpy(element,cRow.Extract(j+1));
		      short type = Evaluate(itfc,RESPECT_ALIAS,element,result);
            if(type == FLOAT32)
