...
STDMETHODIMP AXClientSite :: OnShowWindow(BOOL f)
      {
      MyInvalidateRect(Window, 0, TRUE);
      MyInvalidateRect(Parent, 0, TRUE);
      return S_OK;
      }
...
extern "C" __declspec(dllexport) void TextMessage(const char *const text,...);
// Window Procedure for AX control
LRESULT CALLBACK AXWndProc(HWND hh,UINT mm,WPARAM ww,LPARAM ll)
{
  // Sleep(10);
 //  TextMessage("Event = %X\n",mm);
      if(mm == WM_MOUSEACTIVATE)
      {
         HWND hwndTopLevel = (HWND) ww;       // handle of top-level parent 
         SetHTMLViewerFocus(hh,hwndTopLevel);
      }
     ...
     // }
      ...
      if(mm == WM_CREATE)
      {
         ...
         SetWindowLong(hh,GWL_USERDATA,(LONG)wi);
...
void DestroyAX(HWND hWnd)
{
   AX* ax = GetAXData(hWnd);
   if(ax)
   {
