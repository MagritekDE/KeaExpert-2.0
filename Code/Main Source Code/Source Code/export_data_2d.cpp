...
// Export the data **********************************************   
   return(Export2DData(itfc,file,exportType,matrixName.Str()));
}
...
UINT Export2DHookProc(HWND hdlg,UINT uiMsg,WPARAM wParam,LPARAM lParam);

short Export2DDataDialog(HWND hWnd, char *pathName, char *fileName)
{
   short err;
   static short index;
...
// Save the current working directory as the following command changes it
   ...
   if(IsPlacesBarHidden())
   {
      err = FileDialog(hWnd, false, pathName, fileName, 
                               "Export 2D data", Export2DHookProc, "EXPORT2DDLG", 
                                templateFlag, 1, &index,  "All files", "*");
   }
   else
   {
      err = FileDialog(hWnd, false, pathName, fileName, 
                               "Export 2D data", Export2DHookProc, "EXPORT2DDLG2", 
                                templateFlag, 1, &index,  "All files", "*");
   }
...
UINT Export2DHookProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
		...
		case(WM_NOTIFY): // Process events from standard part of dialog
		{
		   ...
		   switch(lpon->hdr.code)
		   {
            ...
            // Extract matrixName variable name
               ...
               Interface itfc;
	       	   if(Export2DData(&itfc,file,exportType,matrixName) == ERR)
	       	   {
...
/************************************************************************
*                  Export 2D data as specified type                     *
************************************************************************/

short Export2DData(Interface *itfc, char fileName[], long type, char* matrixName)
{
   Variable *varMat;
   float **rMat = NULL;
   complex **cMat= NULL;
...
// Get data from variables *******************************************
   ...
   if(type & REAL_DATA)
   {
		if(varMat->GetType() != MATRIX2D)
		{
		   ErrorMessage("Variable '%s' is not a real matrix",matrixName);
		   return(ERR);
		}		
      rMat = VarRealMatrix(varMat);
   }

   else if(type & COMP_DATA)
   {
		if(varMat->GetType() != CMATRIX2D)
		{
		   ErrorMessage("Variable '%s' is not a complex matrix",matrixName);
		   return(ERR);
		}		
      cMat = VarComplexMatrix(varMat);
   }	 

	xSize = VarColSize(varMat);
	ySize = VarRowSize(varMat);
	   			   
   if(type & BINARY)
   {
      if(Save2DDataToBinaryFile(fileName, type, rMat, cMat, xSize, ySize) == ERR)
         return(ERR);
   }
   else if(type & ASCII)
   {
      if(Save2DDataToAsciiFile(fileName, type, rMat, cMat, xSize, ySize) == ERR)
         return(ERR);
   }
   return(OK);
}
