...
/***************************************************************************
   Make a list box object with extended selection
***************************************************************************/

void MakeListBoxEsObject(ObjectData* obj, long x, long y, long w, long h, DWORD visibility)
{
   obj->hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "listbox", "", WS_CHILD | LBS_NOTIFY | WS_VSCROLL | LBS_NOINTEGRALHEIGHT | LBS_HASSTRINGS | visibility | LBS_OWNERDRAWFIXED | LBS_EXTENDEDSEL,
      x, y, w, h,
      obj->hwndParent, (HMENU)obj->nr(),
      prospaInstance, NULL);
   OldListBoxProc = (WNDPROC)SetWindowLong(obj->hWnd, GWL_WNDPROC, (LONG)ListBoxEventProc);
     ...
     // Draw the background for this row ----------------------------------------
         ...
         { 
            ...
            { 
               ...
               if(!info->allowExtendedSelections && (info->firstLineSelected != info->lastLineSelected))
               {
            ...
            else // Otherwise set the background to white unless this is a multiple row selection
            {
               ...
               if (!info->allowExtendedSelections && (info->firstLineSelected != info->lastLineSelected))
               {
                  ...
                  // Inside selection
                  else if ((pdis->itemID >= info->firstLineSelected && pdis->itemID <= info->lastLineSelected) || // Normal selection
                     (pdis->itemID <= info->firstLineSelected && pdis->itemID >= info->lastLineSelected)) // Reverse selection
                  {
                     HBRUSH backBrush = CreateSolidBrush(GetSysColor(COLOR_BTNFACE));
                     FillRect(pdis->hDC, &r, backBrush);
                  }
               ...
               else // This is a normal single selection which is decorated elsewhere
               {
                  FillRect(pdis->hDC,&r,(HBRUSH)GetStockObject(WHITE_BRUSH));
                  if(pdis->itemID == nrItems-1) 
                  {
   ...
   switch(messg)
   {
     ...
     // Note location of cursor in client coordinates
      ...
      case(WM_LBUTTONDOWN): // Left click down
      {
         ...
         if(wParam & MK_CONTROL)  // Could be part of a multiple selection
         {
            if (info->allowExtendedSelections) // In this case just select the row but don't load the data
            {
               int selectCount = SendMessage(obj->hWnd, LB_GETSELCOUNT, 0, 0);
               if (selectCount > 0)
               {
...
int SetListBoxParameter(ObjectData *obj, CText &parameter, Variable *value)
{
   ...
   {
      ListBoxInfo* info = (ListBoxInfo*)obj->data;
      if (info->allowExtendedSelections) // For LISTBOX_ES (use an array to select items)
      {
         if (type == NULL_VARIABLE) // Remove all selections
         {
            SendMessage(obj->hWnd, LB_SETSEL, FALSE, -1);
         ...
         else if (type == MATRIX2D)
         {
            if (value->GetDimY() == 1)
            {
               float** selectArray = value->GetMatrix2D();
               int size = value->GetDimX();
               int selectCount = SendMessage(obj->hWnd, LB_GETCOUNT, 0, 0);
               for (int i = 0; i < size; i++)
               {
                  int selection = (nint)(selectArray[0][i]);
                  if (selection < 0 || selection >= selectCount)
                  {
                     ErrorMessage("Selection vector is out of range");
                     return(ERR);
                  }
                  SendMessage(obj->hWnd, LB_SETSEL, TRUE, selection);
               }
               MyInvalidateRect(obj->hWnd, NULL, true);
            }
            else
            {
               ErrorMessage("Selection argument should be a vector");
               return(ERR);
            }
         }
         else
         {
            ErrorMessage("Selection argument should be a vector or null");
            return(ERR);
         }
      }
      else
      {
         if (type == MATRIX2D)
         {
            if (value->GetDimY() == 1)
            {
               ...
               info->firstLineSelected = nint(selection[0][0]);
               info->lastLineSelected = nint(selection[0][1]);
               MyInvalidateRect(obj->hWnd, NULL, true);
            }
            else
            {
   ...
   else if(parameter == "text") // Select the list entry by text
   {
      ...
      if(type == UNQUOTED_STRING)
      {
         ...
         {
            SendMessage(obj->hWnd, LB_SETSEL, FALSE, -1);
            index = SendMessage(obj->hWnd, LB_SETSEL, TRUE, index);
         }
         else
         {
...
int GetListBoxParameter(Interface *itfc, ObjectData *obj, CText &parameter, Variable *ans)
{
   ...
   {
      ListBoxInfo* info = (ListBoxInfo*)obj->data;
      if (info->allowExtendedSelections) // For LISTBOX_ES return the first selected item in list
      {
         int selectCount = SendMessage(obj->hWnd, LB_GETSELCOUNT, 0, 0);
         if (selectCount > 0)
         {
            int* selectedIndices = new int[selectCount];
            SendMessage(obj->hWnd, LB_GETSELITEMS, (WPARAM)selectCount, (LPARAM)selectedIndices);
   ...
   else if(parameter == "selection") // Return the selection list or range - keep order
   {
      ListBoxInfo* info = (ListBoxInfo*)obj->data;

      if (info->allowExtendedSelections) // For LISTBOX_ES (returns an array of selected items)
      {
         int selectCount = SendMessage(obj->hWnd, LB_GETSELCOUNT, 0, 0);
         ...
         if (selectCount > 0)
         {
            ...
            ans->MakeMatrix2DFromIVector(selectedIndices, selectCount, 1);
      ...
      else // For legacy LISTBOX (returns the selection range [first,last])
      {
