...
int IsFileCached(Interface* itfc, char args[])
{
   ...
   if ((nrArgs = ArgScan(itfc, args, 1, "macroname, macropath, mode", "eee", "ttt", &name, &path, &mode)) < 0)
      return(ERR);
   ...
   if (mode == "window")
   {
      if (itfc->win)
      {
         ...
         for (Variable* var = procList->next; var != NULL; var = var->next)
         {
            ...
            if (path == "")
            {
               if (!stricmp(procInfo->macroName, name.Str()))
               {
                  itfc->retVar[1].MakeAndSetFloat(1);
                  itfc->retVar[2].MakeAndSetString(procInfo->macroPath);
                  itfc->nrRetValues = 2;
                  return(OK);
               }
            }
   ...
   if (path == "")
   {
      ...
      itfc->retVar[2].MakeAndSetString("");
      itfc->nrRetValues = 2;
   }
   else
   {
...
EXPORT Variable* GetProcedure(char* path, char* macro, char* name)
{
